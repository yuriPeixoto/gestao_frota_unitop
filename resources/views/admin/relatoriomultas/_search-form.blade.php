<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.input type="date" name="data_inclusao" label="Placa:" value="{{ request('data_inclusao') }}" />


        <x-forms.input type="date" name="data_final" label="Filial:" value="{{ request('data_final') }}" />
        ...
        <x-forms.smart-select name="id_pessoal" label="Condutor:" placeholder="Selecionar" :options="$condutor"
            value="{{ request('id_pessoal') }}" :searchUrl="route('admin.api.motorista.search')" asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriomultas.gerarPdf()"
                :disabled="$store.relatoriomultas.loading"
                ...
                <span x-show="$store.relatoriomultas.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriomultas.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriomultas.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriomultas.gerarExcel()"
                :disabled="$store.relatoriomultas.loading"
                ...
                <span x-show="$store.relatoriomultas.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriomultas.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriomultas.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
