<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioveiculos', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tipo_equipamento: document.querySelector('select[name="id_tipo_equipamento"], #id_tipo_equipamento')?.value || '',
                    id_veiculo: document.querySelector('select[name="id_veiculo"], #id_veiculo')?.value || '',
                    id_pessoal: document.querySelector('select[name="id_pessoal"], #id_pessoal')?.value || '',
                    situacao_veiculo: document.querySelector('select[name="situacao_veiculo"], #situacao_veiculo')?.value || '',
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioveiculos/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_veiculos${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioveiculos/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_veiculos_v2${new Date().toISOString().slice(0,10)}.xls`);
