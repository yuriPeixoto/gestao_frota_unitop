<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioabastecimentototais', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tipo_combustivel: document.querySelector('select[name="id_tipo_combustivel"], #id_tipo_combustivel')?.value || '',
                    id_tipo_abastecimento: document.querySelector('select[name="id_tipo_abastecimento"], #id_tipo_abastecimento')?.value || '',
                };
            },
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioabastecimentototais/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_abastecimento_totais${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioabastecimentototais/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_abastecimento_totais${new Date().toISOString().slice(0,10)}.xls`);
