<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorionfsmanutencaorealizadas', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_nf_ordem: document.querySelector('select[name="id_nf_ordem"], #id_nf_ordem')?.value || '',
                    id_filial: document.querySelector('select[name="id_filial"], #id_filial')?.value || '',
                    id_fornecedor: document.querySelector('select[name="id_fornecedor"], #id_fornecedor')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorionfsmanutencaorealizadas/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_realizada${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorionfsmanutencaorealizadas/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_realizada${new Date().toISOString().slice(0,10)}.xls`);
