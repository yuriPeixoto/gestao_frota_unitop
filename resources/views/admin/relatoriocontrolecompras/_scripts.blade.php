<script>
    ...
    Alpine.store('relatoriocontrolecompras', {
        loading: false,
        ...
        async gerarPdf() {
            ...
            try {
                ...
                // Chamada requisição
                const response = await this.fazerRequisicao('/admin/relatoriocontrolecompras/gerarpdf', dados);
                const blob = await response.blob();

                this.fazerDownload(blob, `controle_compras${new Date().toISOString().slice(0,10)}.pdf`);
        ...
        async gerarExcel() {
            ...
            try {
                ...
                // Chamada requisição
                const response = await this.fazerRequisicao('/admin/relatoriocontrolecompras/gerarexcel', dados);
                const blob = await response.blob();

                this.fazerDownload(blob, `controle_compras_v2${new Date().toISOString().slice(0,10)}.xls`);
