<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriopneusaplicado', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_base_veiculo: document.querySelector('select[name="id_base_veiculo"], #id_base_veiculo')?.value || '',
                };
            },
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusaplicado/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_Aplicados${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusaplicado/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_Aplicados${new Date().toISOString().slice(0,10)}.xls`);
