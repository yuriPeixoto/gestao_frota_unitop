<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriosaidadepartamento', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_solicitacao_pecas: document.querySelector('select[name="id_solicitacao_pecas"], #id_solicitacao_pecas')?.value || '',
                };
            },
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosaidadepartamento/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `saida_por_departamento${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosaidadepartamento/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `saida_por_departamento${new Date().toISOString().slice(0,10)}.xls`);
