<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_produto" label="Descrição Produto:" placeholder="Selecionar" :options="$produto"
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioprodutoscadastrados.gerarPdf()"
                :disabled="$store.relatorioprodutoscadastrados.loading"
                ...
                <span x-show="$store.relatorioprodutoscadastrados.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioprodutoscadastrados.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioprodutoscadastrados.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioprodutoscadastrados.gerarExcel()"
                :disabled="$store.relatorioprodutoscadastrados.loading"
                ...
                <span x-show="$store.relatorioprodutoscadastrados.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioprodutoscadastrados.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioprodutoscadastrados.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
