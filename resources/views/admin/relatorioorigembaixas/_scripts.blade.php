<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioorigembaixas', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioorigembaixas/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Relatorio_origem_baixas_pecas${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioorigembaixas/gerarexcel', dados);
                    const blob = await response.blob();
