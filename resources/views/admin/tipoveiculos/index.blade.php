<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <div class="flex items-center space-x-4">
                <a href="{{ route('admin.tipoveiculos.create') }}"
                ...
                <div class="relative inline-block text-left" x-data="{ helpOpen: false }">
                    ...
                    <div x-show="helpOpen" @click.away="helpOpen = false"
                        class="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white 1 ring-black ring-opacity-5">
                        <div class="py-1">
                            <div class="px-4 py-2">
                                <p class="text-sm leading-5 font-medium text-gray-900 truncate">Ajuda - Tipo de Veiculo
                                </p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode visualizar todos os tipos de veículo cadastrados.
                                    Utilize o botão 'Novo Tipo' para adicionar um novo registro.
                                    Você pode editar ou excluir tipos existentes utilizando as
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-4 bg-white border-b border-gray-200">
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <!-- Search Form -->
                @include('admin.tipoveiculos._search-form')
                ...
                <div class="mt-6 overflow-x-auto relative min-h-[400px]">
                    ...
                    <div id="table-loading"
                        ...
                        <div class="flex items-center">
                            <svg class="animate-spin -ml-1 mr-3 h-8 w-8 text-indigo-600"
                    ...
                    <x-tables.table>
                        <x-tables.header>
                            <x-tables.head-cell>Cód. Tipo Veiculo</x-tables.head-cell>
                        ...
                        <x-tables.body>
                            ...
                            <x-tables.row :index="$index" data-id="{{ $tipoveiculo->id }}">
                                ...
                                <x-tables.cell>
                                    <div class="flex items-center space-x-2">
                                        <button type="button" onclick="editarTipoVeiculo({{ $tipoveiculo->id }})"
                                        ...
                                        <button type="button"
                                            onclick="confirmarExclusao({{ $tipoveiculo->id }}, '{{ $tipoveiculo->descricao }}')"
    ...
    <script>
        ...
        // Função para editar tipo de veículo
        function editarTipoVeiculo(id) {
            window.location.href = `{{ route('admin.tipoveiculos.edit', ':id') }}`.replace(':id', id);
        }
        ...
        function executarExclusao() {
            ...
            fetch(`{{ route('admin.tipoveiculos.destroy', ':id') }}`.replace(':id', idExclusao), {
                method: 'DELETE',
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    return response.json().then(data => {
                        throw new Error(data.notification ? data.notification.message : `Erro HTTP: ${response.status}`);
                    });
                }
            ...
            .then(data => {
                ...
                // Mostrar notificação
                showNotification(
                    data.notification ? data.notification.title : 'Sucesso',
                    data.notification ? data.notification.message : 'Tipo de veículo excluído com sucesso',
                    data.notification ? data.notification.type : 'success'
                );
            ...
            .catch(error => {
                ...
                showNotification(
                    'Erro',
                    error.message || 'Não foi possível excluir o tipo de veículo',
                    'error'
                );
            })
            .finally(() => {
