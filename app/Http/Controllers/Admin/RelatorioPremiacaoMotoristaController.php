...
class RelatorioPremiacaoMotoristaController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.relatoriopremiacaomotorista.index', compact('motorista', 'filial', 'placa'));
    }
    ...
    public function gerarPdf(Request $request)
    {
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            ...
            if (empty($request['id_motorista'])) {
                ...
                $in_equipamento  = 'IN';
                $id_equipamento  = implode(",", $request['id_motorista']);
            }
            if (empty($request['id_veiculo'])) {
                ...
                $in_veiculo  = 'IN';
                $id_veiculo  = implode(",", $request['id_veiculo']);
            }
            ...
            $name       = 'new_historico_premiacao_v7';
