<x-app-layout>
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                ...
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode cadastrar um novo fornecedor. Preencha todos os
                                    campos obrigatórios na parte superior e adicione uma ou mais informações nas abas
                                    disponíveis.
                                </p>
                            </div>
                        </div>
    ...
    <form id="form-fornecedor" action="{{ route('admin.fornecedores.store') }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @method('POST')

        <!-- Campo oculto para ID do fornecedor (para JavaScript) -->
        <input type="hidden" id="contrato_fornecedor_fornecedor_id_fornecedor" value="">

        @include('admin.fornecedores._form')
    </form>

    @push('scripts')
    <!-- Scripts para gerenciamento de contratos e relacionamentos -->
    <script src="{{ asset('js/contratos.js') }}"></script>
    <script src="{{ asset('js/contrato-modelo.js') }}"></script>
    {{--<script src="{{ asset('js/servicos.js') }}"></script>--}}


    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Notificações de sucesso/erro guardadas em sessão
            @if(session('success'))
