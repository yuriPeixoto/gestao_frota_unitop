<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioindicecoberturaestoque', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioindicecoberturaestoque/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `indice_cobertura_estoque${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioindicecoberturaestoque/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `indice_cobertura_estoque${new Date().toISOString().slice(0,10)}.xls`);
