<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriolistagempneusmanutencao', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tipo_reforma: document.querySelector('select[name="id_tipo_reforma"], #id_tipo_reforma')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriolistagempneusmanutencao/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_manutencao${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriolistagempneusmanutencao/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_manutencao_v2${new Date().toISOString().slice(0,10)}.xls`);
