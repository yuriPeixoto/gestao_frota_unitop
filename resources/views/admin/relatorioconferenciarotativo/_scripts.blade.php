<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioconferenciarotativo', {
            loading: false,

            //Aqui colcamos os parâmetros de acordo com o html
            coletarDadosFormulario() {
                return {
                    data_baixa: document.querySelector('select[name="data_baixa"], #data_baixa')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioconferenciarotativo/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `PlaIpva_v2022_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioconferenciarotativo/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `PlaIpva_v2022_v2${new Date().toISOString().slice(0,10)}.xls`);
