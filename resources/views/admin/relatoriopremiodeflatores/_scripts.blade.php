<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriopremiodeflatores', {
            loading: false,
            //Aqui colcamos os parâmetros de acordo com o html
            coletarDadosFormulario() {
                return {
                    nome: document.querySelector('input[name="nome"], #nome')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopremiodeflatores/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_premio_deflatores_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopremiodeflatores/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_premio_deflatores_v2${new Date().toISOString().slice(0,10)}.xls`);
