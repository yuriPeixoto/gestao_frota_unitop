<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_tanque" label="Tanque:" placeholder="Selecionar" :options="$tanque"
            value="{{ request('id_tanque') }}" asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            {{-- <button type="button" x-on:click="$store.relatoriorecebimentocombustivel.gerarPdf()"
                :disabled="$store.relatoriorecebimentocombustivel.loading"
                ...
                <span x-show="$store.relatoriorecebimentocombustivel.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriorecebimentocombustivel.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriorecebimentocombustivel.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button> --}}

            <button type="button" x-on:click="$store.relatoriorecebimentocombustivel.gerarExcel()"
                :disabled="$store.relatoriorecebimentocombustivel.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatoriorecebimentocombustivel.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriorecebimentocombustivel.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
