<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriolistagempneusdescartados', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tipo_descarte: document.querySelector('select[name="id_tipo_descarte"], #id_tipo_descarte')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriolistagempneusdescartados/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `descarte_pneu${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriolistagempneusdescartados/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `descarte_pneu_v2${new Date().toISOString().slice(0,10)}.xls`);
