...
class RelatorioBaixaEstoque extends Controller
{
    public function index(Request $request)
    {
        $query = Produto::query();
        $query2 = Departamento::query();
        ...
        if ($request->filled('id_departamento')) {
            $query2->where('id_departamento', $request->input('id_departamento'));
        }
        ...
        return view('admin.relatoriobaixaestoque.index', compact('filial', 'produto', 'estoque', 'grupo', 'departamento'));
    }
        ...
        try {
            ...
            if ($request->filled(['data_inclusao', 'data_final'])) {
                ...
                if (empty($request['id_estoque'])) {
                    $P_id_estoque_produto_inicial = 0;
                    $P_id_estoque_produto_final   = 99999999;
                } else {
                    $P_id_estoque_produto_inicial = $request['id_estoque'];
                    $P_id_estoque_produto_final   = $request['id_estoque'];
                }
                ...
                if (empty($request['id_departamento'])) {
                    ...
                    $id_departamento_final   = 99999999;
                } else {
                ...
                $parametros = array(
                    ...
                    'P_filial_inicial'                      => $id_filial_inicial,
                    'P_filial_final'                        => $id_filial_final,
                    'P_id_estoque_produto_inicial'          => $P_id_estoque_produto_inicial,
                    'P_id_estoque_produto_final'            => $P_id_estoque_produto_final,
    ...
    public function gerarExcel(Request $request)
    {
        try {
            ...
            if ($request->filled(['data_inclusao', 'data_final'])) {
                ...
                $name       = 'relatorio_baixa_estoque_v2';
                $agora = now()->format('d-m-Y_H-i');
