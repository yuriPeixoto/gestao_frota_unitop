<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Unidades de Produto') }}
            </h2>
            <div class="flex items-center space-x-4">
                {{-- Botão Criar - Protegido por Permissão --}}
                @can('criar_unidadeproduto')
                <a href="{{ route('admin.unidadeprodutos.create') }}"
                ...
                <div class="relative inline-block text-left" x-data="{ helpOpen: false }">
                    ...
                    <div x-show="helpOpen" @click.away="helpOpen = false"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                <p class="text-sm leading-5 font-medium text-gray-900 truncate">Ajuda - Unidades de
                                    Produto</p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode visualizar e gerenciar todas as unidades de produto cadastradas
                                    no sistema.
                                    Use o botão "Nova Unidade" para adicionar novos registros.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="p-4 bg-white border-b border-gray-200">
            ...
            <div class="overflow-x-auto relative">
                <x-tables.table>
                    ...
                    <x-tables.body>
                        @forelse ($unidadeProdutos as $index => $unidade)
                        <x-tables.row :index="$index" data-id="{{ $unidade->id_unidade_produto }}">
                            <x-tables.cell>{{ $unidade->id_unidade_produto }}</x-tables.cell>
                            <x-tables.cell>{{ $unidade->descricao_unidade }}</x-tables.cell>
                            <x-tables.cell nowrap>
                                {{ $unidade->data_inclusao ? $unidade->data_inclusao->format('d/m/Y H:i') : '' }}
                            </x-tables.cell>
                            <x-tables.cell nowrap>
                                {{ $unidade->data_alteracao ? $unidade->data_alteracao->format('d/m/Y H:i') : '' }}
                            </x-tables.cell>
                            ...
                            <x-tables.cell>
                                <div class="flex items-center space-x-2">
                                    {{-- Botão Visualizar --}}
                                    <a href="{{ route('admin.unidadeprodutos.show', $unidade->id_unidade_produto) }}"
                                    ...
                                    <a href="{{ route('admin.unidadeprodutos.edit', $unidade->id_unidade_produto) }}"
                                    ...
                                    {{-- Botão Excluir --}}
                                    @can('excluir_unidadeproduto')
                                    <form method="POST"
                                        action="{{ route('admin.unidadeprodutos.destroy', $unidade->id_unidade_produto) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                        <x-tables.empty cols="5" message="Nenhuma unidade de produto encontrada" />
                        @endforelse
                    </x-tables.body>
                </x-tables.table>
    ...
    {{-- <div x-data="{
        ...
        async excluirUnidade() {
            try {
                const response = await fetch(`{{ route('admin.unidadeprodutos.destroy', ':id') }}`.replace(':id', this.unidadeId), {
                    method: 'DELETE',
                    headers: {
                ...
                if (data.success) {
                    ...
                    const row = document.querySelector(`tr[data-id=\'\${this.unidadeId}\']`);
                    if (row) {
                ...
                alert('Erro: Não foi possível excluir a unidade de produto.');
            }
        }
    }" @confirmar-exclusao.window="showModal = true; unidadeId = $event.detail.id; unidadeNome = $event.detail.nome">
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Tem certeza que deseja excluir a unidade de produto
                                        <strong x-text="unidadeNome"></strong>?
                                        <br><br>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="button" @click="excluirUnidade()"
