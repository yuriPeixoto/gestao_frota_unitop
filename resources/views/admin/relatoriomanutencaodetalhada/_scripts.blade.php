<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioManutencaoDetalhada', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_departamento: document.querySelector('select[name="id_departamento"], #id_departamento')?.value || '',
                    id_categoria: document.querySelector('select[name="id_categoria"], #id_categoria')?.value || '',
                    id_manutencao: document.querySelector('select[name="id_manutencao"], #id_manutencao')?.value || '',
                    id_ordem_servico: document.querySelector('select[name="id_ordem_servico"], #id_ordem_servico')?.value || '',
                    id_tipo_ordem_servico: document.querySelector('select[name="id_tipo_ordem_servico"], #id_tipo_ordem_servico')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomanutencaodetalhada/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_detalhada_${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomanutencaodetalhada/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_detalhada_${new Date().toISOString().slice(0,10)}.xls`);
