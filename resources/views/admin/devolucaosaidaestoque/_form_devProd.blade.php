<div class="space-y-6">
    ...
    <div class="p-6 bg-white border-b border-gray-200">
        <form id="devolucaoProdutos" method="POST" action="{{ $action }}" class="space-y-4"
            ...
            <div class="flex gap-4">
                <div class="w-32 flex-none">
                    <x-forms.input name="id_devolucao_produtos" label="Código Devolução" readonly
                        value="{{ old('id_devolucao_produtos', $devulucaoProdutos->id_devolucao_produtos ?? '') }}" />
                </div>
                <div class="w-32 flex-1">
                    <x-forms.smart-select name="id_filial" label="Filial" placeholder="Selecione a filial..."
                        :options="$filiais" :selected="old('id_filial', $devulucaoProdutos->id_filial ?? '')" asyncSearch="false" />
                </div>
                <div class="w-32 flex-auto">
                    <x-forms.smart-select name="id_ordem_servico" label="Ordem de Serviço"
                        placeholder="Selecione a ordem de serviço..." :options="$ordem_servico" :selected="old('id_fornecedor', $manutencaoPneus->id_fornecedor ?? '')"
                        asyncSearch="true" />
                </div>
                <div class="w-32 flex-auto">
                    <x-forms.smart-select name="id_produto" label="Descrição Produto" :options="[]"
                        :searchUrl="route('admin.api.produto.search')" :selected_value="old('id_produto', $devulucaoProdutos->id_produto ?? '')" asyncSearch="true" />
                </div>
                <div class="w-32 flex-auto">
                    <x-forms.input name="quantidade" label="Quantidade"
                        value="{{ old('quantidade', $devulucaoProdutos->quantidade ?? '') }}" />
                </div>
            </div>
