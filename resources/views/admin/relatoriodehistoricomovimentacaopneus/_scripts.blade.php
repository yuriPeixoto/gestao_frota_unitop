<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriodehistoricomovimentacaopneus', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_controle_vida_pneu: document.querySelector('select[name="id_controle_vida_pneu"], #id_controle_vida_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriodehistoricomovimentacaopneus/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_Aplicados_Historico${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriodehistoricomovimentacaopneus/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_Aplicados_Historico_v2${new Date().toISOString().slice(0,10)}.xls`);
