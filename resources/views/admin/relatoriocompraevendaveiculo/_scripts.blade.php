<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriocompraevendaveiculo', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    status_pneu: document.querySelector('select[name="status_pneu"], #status_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocompraevendaveiculo/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Relatorio_contrato_fornecedores${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocompraevendaveiculo/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Relatorio_contrato_fornecedores${new Date().toISOString().slice(0,10)}.xls`);
