...
class RelatorioPremioConferenciaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $filial = Filial::select('id as value', 'name as label')
            ...
            ->limit(30)
            ->get();
        $subcat = SubCategoriaVeiculo::select('id_subcategoria as value', 'descricao_subcategoria as label')
            ->orderBy('descricao_subcategoria')
            ->limit(30)
            ->get();
        return view('admin.relatoriopremioconferencia.index', compact('motorista', 'filial', 'subcat'));
    }
    ...
    public function gerarPdfSecundo(Request $request)
    {
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
    ...
    public function gerarExcel(Request $request)
    {
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            ...
            $name       = 'historico_premiacao_tabelao';
            $agora = now()->format('d-m-Y_H-i');
