<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="nome" label="Motorista:" value="{{ request('nome') }}" :options="$motorista" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriopremiodeflatores.gerarPdf()"
                :disabled="$store.relatoriopremiodeflatores.loading"
                ...
                <span x-show="$store.relatoriopremiodeflatores.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopremiodeflatores.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopremiodeflatores.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriopremiodeflatores.gerarExcel()"
                :disabled="$store.relatoriopremiodeflatores.loading"
                ...
                <span x-show="$store.relatoriopremiodeflatores.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopremiodeflatores.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopremiodeflatores.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
