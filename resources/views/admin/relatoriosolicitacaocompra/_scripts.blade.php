<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriosolicitacaocompra', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosolicitacaocompra/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_solicitacao${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosolicitacaocompra/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_solicitacao${new Date().toISOString().slice(0,10)}.xls`);
