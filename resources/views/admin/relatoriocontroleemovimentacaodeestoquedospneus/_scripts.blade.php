<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriocontroleemovimentacaodeestoquedospneus', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_pneu: document.querySelector('select[name="id_pneu"], #id_pneu')?.value || '',
                    id_modelo_pneu: document.querySelector('select[name="id_modelo_pneu"], #id_modelo_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocontroleemovimentacaodeestoquedospneus/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `controle_movimentacao_pneu${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocontroleemovimentacaodeestoquedospneus/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `controle_movimentacao_pneu${new Date().toISOString().slice(0,10)}.xls`);
