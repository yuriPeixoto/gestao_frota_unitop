<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioservicosfornecedores', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_servico: document.querySelector('select[name="id_servico"], #id_servico')?.value || '',
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioservicosfornecedores/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_servico_forn${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioservicosfornecedores/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_servico_forn_v2${new Date().toISOString().slice(0,10)}.xls`);
