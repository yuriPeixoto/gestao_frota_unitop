<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('historicoManuntencao', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/historicomanutencaoveiculo/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_veiculo_${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/historicomanutencaoveiculo/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_manutencao_veiculo_${new Date().toISOString().slice(0,10)}.xls`);
