<x-app-layout>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.usuarios.store') }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <x-forms.input name="cpf" label="CPF" value="{{ old('cpf') }}" />
                            <x-forms.input name="matricula" type="number" label="Matrícula" value="{{ old('matricula') }}" />
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-forms.input name="password" type="password" label="Senha" required />
                            <x-forms.input name="password_confirmation" type="password" label="Confirmar Senha" required />
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-forms.smart-select name="filiais[]" label="Filiais" placeholder="Selecione as Filiais" :options="$filiais ?? []" multiple="true" />
                            <x-forms.smart-select name="departamento_id" label="Departamento" placeholder="Selecione o Departamento" :options="$departamentos ?? []" />
                            <x-forms.smart-select name="pessoal_id" label="Cargo" placeholder="Selecione o Cargo" :options="$cargos ?? []" />
                            <x-forms.smart-select name="roles[]" label="Grupos (Roles)" placeholder="Selecione os grupos" :options="$grupos ?? []" multiple="true" />
                        </div>
                        <p class="text-xs text-gray-500 -mt-4">Dica: a primeira filial selecionada será considerada a principal.</p>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-forms.input name="telefone_fixo" label="Telefone Fixo" value="{{ old('telefone_fixo') }}" />
                            <x-forms.input name="telefone_celular" label="Telefone Celular" value="{{ old('telefone_celular') }}" />
                        </div>

                        <div class="border-t pt-4 mt-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Endereço (opcional)</h3>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <x-forms.input name="zip_code" label="CEP" value="{{ old('zip_code') }}" />
                                <x-forms.input name="state" label="UF" maxlength="2" value="{{ old('state') }}" />
                                <x-forms.input name="city" label="Cidade" value="{{ old('city') }}" />
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                                <x-forms.input name="street" label="Logradouro" value="{{ old('street') }}" />
                                <x-forms.input name="number" label="Número" value="{{ old('number') }}" />
                                <x-forms.input name="complement" label="Complemento" value="{{ old('complement') }}" />
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                <x-forms.input name="district" label="Bairro" value="{{ old('district') }}" />
                            </div>
                        </div>
    ...
    <script>
        (function() {
            ...
            // Elements
            const form = document.querySelector('form[action*="admin.usuarios.store"], form[action*="/admin/usuarios"]') || document.querySelector('form');
            ...
            // Before submit, strip non-digits so backend receives clean values
            if (form){
