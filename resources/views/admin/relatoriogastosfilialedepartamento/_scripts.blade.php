<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriogastosfilialedepartamento', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriogastosfilialedepartamento/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `gastos_filial_departamento${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriogastosfilialedepartamento/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `gastos_filial_departamento${new Date().toISOString().slice(0,10)}.xls`);
