<form method="GET" action="{{ route('admin.compras.notas-lancadas.index') }}" class="space-y-4">
    ...
    <div class="flex w-full gap-4">
        ...
        <div class="w-full">
            <x-forms.input type="date" name="data_solicitacao" label="Data Solicitação" />
        </div>

        <div class="w-full">
            <x-forms.input type="date" name="data_emissao" label="Data Data Emissão" />
        </div>



        <div class="w-full">
            <x-forms.smart-select name="id_solicitante" label="Solicitante:" :options="$solicitantes" />
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="w-full">
            <x-forms.input name="id_nf_avulsa" label="Cód:" />
        </div>
        ...
        <div class="w-full">
            ...
            <x-forms.input name="serie_nf" label="Série:" />
        </div>
    ...
    <div>
        ...
        <a href="{{ route('admin.compras.notas-lancadas.exportPdf', request()->only(['id', 'numero_nf', 'nome_fornecedor'])) }}"
        ...
        {{-- Usar o novo componente de botões de exportação --}}
        <a href="{{ route('admin.compras.notas-lancadas.exportCsv', request()->only(['id', 'numero_nf', 'nome_fornecedor'])) }}"
        ...
        {{-- Usar o novo componente de botões de exportação --}}
        <a href="{{ route('admin.compras.notas-lancadas.exportXls', request()->only(['id', 'numero_nf', 'nome_fornecedor'])) }}"
            class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 export-btn">
        ...
        <a href="{{ route('admin.compras.notas-lancadas.exportXml', request()->only(['id', 'numero_nf', 'nome_fornecedor'])) }}"
            class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 export-btn">
    ...
    document.querySelectorAll('.export-btn').forEach(function(btn) {
        btn.addEventListener('click', function(event) {
            ...
            document.querySelectorAll('.pedido-checkbox:checked').forEach(cb => {
                selecionados.push(`id[]=${encodeURIComponent(cb.value)}`);
            });
