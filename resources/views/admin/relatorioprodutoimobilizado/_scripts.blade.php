<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioprodutoimobilizado', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoimobilizado/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `produtos_imobilizados_v3${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoimobilizado/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `produtos_imobilizados_v3${new Date().toISOString().slice(0,10)}.xls`);
