<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="numero_nf" label="N° Nota Fiscal" placeholder="Selecionar" :options="$numeroNota"
            value="{{ $numeroNotaFiltro }}" :searchUrl="route('admin.api.notasfiscais.search')" asyncSearch="false" />

        <div>
            <x-forms.input type="number" name="id_ordem_servico" label="N° Ordem de Serviço:"
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioSinteticoNfOs.gerarPdf()"
                :disabled="$store.relatorioSinteticoNfOs.loading"
                ...
                <span x-show="$store.relatorioSinteticoNfOs.loading" class="loading-spinner mr-2"></span>
                <!-- Ícone normal (quando não carregando) -->
                <x-icons.magnifying-glass x-show="!$store.relatorioSinteticoNfOs.loading" class="h-4 w-4 mr-2" />

                <!-- Texto do botão -->
                <span x-text="$store.relatorioSinteticoNfOs.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioSinteticoNfOs.gerarExcel()"
                :disabled="$store.relatorioSinteticoNfOs.loading"
                ...
                <!-- Texto do botão -->
                <span x-text="$store.relatorioSinteticoNfOs.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
        </div>
