...
class RelatorioPneusAplicado extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('id_base_veiculo')) {
            $query->where('id_base_veiculo', $request->input('id_base_veiculo'));
        }
        ...
        $base = BaseVeiculo::select('id_base_veiculo as value', 'descricao_base as label')
            ->orderBy('descricao_base')
            ->limit(30)
            ->get();

        return view('admin.relatoriopneusaplicado.index', compact(
            'filiais',
            'departamento',
            'placa',
            'base'
    ...
    public function gerarPdf(Request $request)
    {
        ...
        $base = empty($request['id_base_veiculo']) ? '0' : $request['id_base_veiculo'];
        ...
        if (empty($request['id_base_veiculo'])) {
            $in_base = '!=';
            $id_base = '0';
        } else {
            $in_base = 'IN';
            $id_base = $request['id_base_veiculo'];
        }
