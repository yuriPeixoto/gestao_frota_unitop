<x-bladewind::card reduce_padding="true">
    <div>
        <x-bladewind::table selectable="false" checkable="false" name="controleFrota">
            ...
            <x-slot name="header">
                <th class="text-sm centralizado">Cód. Planejamento</th>
            ...
            @forelse ($servico as $controle)
                <tr data-id="{{ $controle->id_planejamento_manutencao }}" class="text-xs cursor-pointer">
                    <td>{{ $controle->id_planejamento_manutencao }}</td>
                    ...
                    <td class="whitespace-pre-wrap">{{ $controle->id_manutencao ? $controle->manutencao->descricao_manutencao : '' }}</td>
                    <x-tables.cell>
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('admin.manutencaoservico.edit', $controle->id_planejamento_manutencao) }}"
                                class="inline-flex items-center p-1 border border-transparent rounded-full shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                <x-icons.pencil class="h-3 w-3" />
                            </a>

                            @if (auth()->user()->id == 1 || in_array(auth()->user()->id, [2, 3, 4, 25]))
                                <button type="button" onclick="confirmarExclusao({{ $controle->id_planejamento_manutencao }})"
