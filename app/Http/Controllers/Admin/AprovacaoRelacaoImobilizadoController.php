...
use App\Models\RelacaoImobilizados;
...
class AprovacaoRelacaoImobilizadoController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        return view(
            'admin.aprovacaorelacaoimobilizado.index',
            compact(
                'relacaoImobilizados',
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            //     'cargo' => $cargo,
            //     'filial' => $filial,
            //     'tipo_cargo' => gettype($cargo),
            //     'tipo_filial' => gettype($filial),
            //     'departamento' => $departamentoRegistro,
            //     'departamento' => $departamento
            // ]);

            if (in_array($userId, $idPermitido)) {
                return $this->carregarViewEdit($id);
            }

            if ($cargo == 30 && $departamento == $departamentoRegistro && $filial == 1) {
                return $this->carregarViewEdit($id);
            }

            return redirect()->route('admin.aprovacaorelacaoimobilizado.index')
    ...
    private function carregarViewEdit($id)
    {
        ...
        return view(
            'admin.aprovacaorelacaoimobilizado.edit',
    ...
    public function onAprovar(Request $request): JsonResponse
    {
        $userId = Auth::id();
        $filial = Auth::user()->filial_id;
        $cargo = $this->buscarCargo($userId); // retorna id do cargo do usuário
        ...
        try {
            ...
            $id = $request->input('id');
            $relacaoImobilizados = RelacaoImobilizados::findOrFail($id);
            ...
            return response()->json([
                ...
                'redirect' => route('admin.aprovacaorelacaoimobilizado.index'),
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function onReprovar(Request $request): JsonResponse
    {
        try {
            ...
            $observacao_lider = $request->input('observacao_lider');
            $aprovado = false;
            $status = 'REPROVADA';
            ...
            $relacaoImobilizados->update([
                'aprovado'              => $aprovado,
                'status'                 => $status,
                'observacao_lider'      => $observacao_lider
            ]);
    ...
    public function buscarDepartamento($userId)
    {
        ...
        // Retorna o departamento (id_departamento)
