<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioprodutoscadastrados', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoscadastrados/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `produtos_cadastrados${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoscadastrados/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `produtos_cadastrados_v2${new Date().toISOString().slice(0,10)}.xls`);
