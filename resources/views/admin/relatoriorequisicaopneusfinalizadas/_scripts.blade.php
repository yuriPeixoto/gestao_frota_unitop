<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriorequisicaopneusfinalizadas', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_requisicao_pneu: document.querySelector('select[name="id_requisicao_pneu"], #id_requisicao_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriorequisicaopneusfinalizadas/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `historico_pneu_requisicao_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriorequisicaopneusfinalizadas/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `historico_pneu_requisicao_v2${new Date().toISOString().slice(0,10)}.xls`);
