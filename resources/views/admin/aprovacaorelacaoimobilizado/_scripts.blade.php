...
<script>
    ...
    window.onAprovar = function(id) {
        ...
        fetch(`{{ route('admin.aprovacaorelacaoimobilizado.aprovar') }}`, {
            method: 'POST',
            headers: {
    ...
    window.onReprovar = function(id) {
        const  observacao_lider = document.getElementById('observacao_lider')

        fetch(`{{ route('admin.aprovacaorelacaoimobilizado.reprovar') }}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
            ...
            body: JSON.stringify({ id: id, observacao_lider: observacao_lider.value })
        })
        .then(response => {
            if (!response.ok) {
                return response.json().then(err => { throw err; });
