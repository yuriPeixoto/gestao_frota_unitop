...
<script>
    function baixarBoletosLote() {
        // Captura todos os inputs hidden gerados com os valores selecionados
        const inputs = document.querySelectorAll('input[name="placa[][]"]');
        const placas = Array.from(inputs).map(input => input.value).filter(Boolean);

        console.log('Placas selecionadas:', placas);

        if (placas.length === 0) {
            showNotification('Erro', 'É necessário selecionar alguma placa.', 'error');
            return;
        }

        const url = `{{ route('admin.listagemmultas.baixarlote') }}?placa=${encodeURIComponent(placas.join(','))}`;
...
<script>
    ...
    function abrirCondutorForm(listagemMulta) {
        multaAtual = listagemMulta;

        // Popula as informações do veículo
        document.querySelector('.placa-info').textContent = listagemMulta.placa || '';
        document.querySelector('.renavam-info').textContent = listagemMulta.renavam || '';
        document.querySelector('.ait-info').textContent = listagemMulta.ait || '';
        ...
        async function indicarMotorista(data) {
            ...
            try {
                const response = await fetch('{{ route("admin.listagemmultas.indicar-motorista") }}', {
                    method: 'POST',
                    headers: {
...
function indicarMotorista(data) {
    ...
    // Fazer a requisição
    fetch('{{ route("admin.listagemmultas.indicar-motorista") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
<script>
    function removerMotorista(ait) {
        ...
        // Fazer a requisição
        fetch('{{ route("admin.listagemmultas.remover-motorista") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
