<div class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioextratoipva.gerarPdf()"
                :disabled="$store.relatorioextratoipva.loading"
                ...
                <span x-show="$store.relatorioextratoipva.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioextratoipva.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioextratoipva.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            {{-- <button type="button" x-on:click="$store.relatorioextratoipva.gerarExcel()"
                :disabled="$store.relatorioextratoipva.loading"
                ...
                <span x-show="$store.relatorioextratoipva.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioextratoipva.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioextratoipva.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button> --}}
