<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_solicitacao_pecas" label="Requisicao Matriz:" placeholder="Selecionar"
            :options="$rfilialm" value="{{ request('id_solicitacao_pecas') }}"
            :searchUrl="route('admin.api.solicitacaopecas.search')" asyncSearch="false" />
        <x-forms.smart-select name="id_solicitacao_pecas" label="Requisicao Filial:" placeholder="Selecionar"
            :options="$rfilial" value="{{ request('id_solicitacao_pecas') }}"
            :searchUrl="route('admin.api.solicitacaopecas.search')" asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriohistoricotransferencia.gerarPdf()"
                :disabled="$store.relatoriohistoricotransferencia.loading"
                ...
                <span x-show="$store.relatoriohistoricotransferencia.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriohistoricotransferencia.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriohistoricotransferencia.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            {{-- <button type="button" x-on:click="$store.relatoriohistoricotransferencia.gerarExcel()"
                :disabled="$store.relatoriohistoricotransferencia.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatoriohistoricotransferencia.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriohistoricotransferencia.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button> --}}
