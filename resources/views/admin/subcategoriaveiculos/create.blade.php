...
<div id="drawer"
    ...
    <div class="p-6 overflow-y-auto h-full flex flex-col">
        ...
        <!-- Título -->
        <h2 class="text-xl font-semibold text-gray-800 mb-6">Novo SubCategoria</h2>

        <!-- Conteúdo -->
        <div id="drawer-content" class="flex-1">
            <form action="{{ route('admin.subcategoriaveiculos.store') }}" method="POST" class="flex flex-col h-full">
                @csrf
                @method('POST')

                <!-- Campo -->
                <div class="mb-6">
                    <label for="descricao_subcategoria"
                ...
                <div class="mt-auto flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        ...
                        {{ isset($subcategoriaveiculo) ? 'Salvar' : 'Salvar' }}
                    </button>
                </div>
            </form>
        </div>
