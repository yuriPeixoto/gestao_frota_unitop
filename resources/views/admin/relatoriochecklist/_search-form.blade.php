<div class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioChecklist.gerarPdf()"
                :disabled="$store.relatorioChecklist.loading"
                ...
                <span x-show="$store.relatorioChecklist.loading" class="loading-spinner mr-2"></span>
                <!-- Ícone normal (quando não carregando) -->
                <x-icons.magnifying-glass x-show="!$store.relatorioChecklist.loading" class="h-4 w-4 mr-2" />

                <!-- Texto do botão -->
                <span x-text="$store.relatorioChecklist.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioChecklist.gerarExcel()"
                :disabled="$store.relatorioChecklist.loading"
                ...
                <!-- Texto do botão -->
                <span x-text="$store.relatorioChecklist.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
        </div>
