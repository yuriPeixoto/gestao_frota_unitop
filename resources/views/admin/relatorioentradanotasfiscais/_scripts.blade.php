<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioentradanotasfiscais', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_nota_fiscal_entrada: document.querySelector('select[name="id_nota_fiscal_entrada"], #id_nota_fiscal_entrada')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioentradanotasfiscais/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `nota_entrada${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioentradanotasfiscais/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `nota_entrada${new Date().toISOString().slice(0,10)}.xls`);
