<script>
    ...
    Alpine.store('fornecedorComissionado', {
        loading: false,
        ...
        coletarDadosFormulario() {
            return {
                ...
                id_fornecedor: document.querySelector('[name="id_fornecedor"]')?.value || '',
                descricao_servico: document.querySelector('[name="descricao_servico"]')?.value || ''
            };
        },
        ...
        async gerarPdf() {
            ...
            try {
                ...
                // Chamada requisição
                const response = await this.fazerRequisicao('/admin/fornecedorescomissionadosrelatorio/gerarpdf', dados);
                const blob = await response.blob();

                this.fazerDownload(blob, `relatorio_fornecedor_comissionado_${new Date().toISOString().slice(0,10)}.pdf`);
        ...
        async gerarExcel() {
            ...
            try {
                ...
                // Chamada requisição
                const response = await this.fazerRequisicao('/admin/fornecedorescomissionadosrelatorio/gerarexcel', dados);
                const blob = await response.blob();

                this.fazerDownload(blob, `relatorio_fornecedor_comissionado_${new Date().toISOString().slice(0,10)}.xls`);
        ...
        // Pode manter seu gerarExcel() sem alterações, aplicando os mesmos conceitos se quiser
    });
});
</script>