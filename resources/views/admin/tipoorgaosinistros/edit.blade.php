    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editar Tipo Órgão') }}: {{ $tipoorgaosinistros->descricao_tipo_orgao }}
            </h2>
            <div class="flex items-center space-x-2">
                <x-button-link href="{{ route('admin.tipoorgaosinistros.index') }}"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                ...
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode editar as informações de um tipo de órgão existente. Modifique
                                    o campo de descrição conforme necessário e clique em 'Atualizar' para salvar as
                                    alterações ou 'Cancelar' para retornar sem salvar.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.tipoorgaosinistros.update', $tipoorgaosinistros) }}" method="POST">
                        @csrf
                        @method('PUT')
                        @include('admin.tipoorgaosinistros._form')
                    </form>
                </div>
            </div>
        </div>
