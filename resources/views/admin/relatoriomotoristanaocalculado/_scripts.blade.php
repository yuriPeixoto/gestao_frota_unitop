<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriomotoristanaocalculado', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomotoristanaocalculado/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_motorista_nao_calcucados${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomotoristanaocalculado/gerarexcel', dados);
                    const blob = await response.blob();
