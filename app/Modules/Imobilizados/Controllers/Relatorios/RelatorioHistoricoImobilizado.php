...
class RelatorioHistoricoImobilizado extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.relatoriohistoricoimobilizado.index', compact('filial', 'produto'));
    }
    ...
    public function gerarPdf(Request $request)
    {
        try {
            if ($request->filled('data_inclusao') && $request->filled('data_final')) {
                ...
                // Filiais
                $filiais = $request->input('id_filial', []);
                if (!empty($filiais)) {
                    if (is_array($filiais)) {
                        $idFilial = implode(',', $filiais);
                    } else {
                        $idFilial = $filiais;
                    }
                    $inFilial = 'IN';
                ...
                // Parâmetros Jasper
                $parametros = [
                    'P_in_filial'    => $inFilial,
                    'P_id_filial'    => $idFilial,
                    'P_in_produto'   => $inProdutos,
                    'P_id_produto'   => $idProdutos,
                ...
                $name       = 'movimentacao_imobilizados';
    ...
    public function gerarExcel(Request $request)
    {
        try {
            if ($request->filled('data_inclusao') && $request->filled('data_final')) {
                ...
                $name       = 'movimentao_imobilizados_v2';
                $agora = now()->format('d-m-Y_H-i');
