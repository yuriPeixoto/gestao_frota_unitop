<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioinventariopneusaplicados', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioinventariopneusaplicados/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `calibragem_vencida_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioinventariopneusaplicados/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `calibragem_vencida_v2${new Date().toISOString().slice(0,10)}.xls`);
