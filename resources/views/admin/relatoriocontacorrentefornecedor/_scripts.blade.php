<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriocontacorrentefornecedor', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocontacorrentefornecedor/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `conta_corrente_fornecedor${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocontacorrentefornecedor/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `conta_corrente_fornecedor_v2${new Date().toISOString().slice(0,10)}.xls`);
