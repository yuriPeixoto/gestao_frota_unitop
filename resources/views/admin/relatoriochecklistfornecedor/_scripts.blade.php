<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriochecklistfornecedor', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriochecklistfornecedor/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `checklist_forncedor${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriochecklistfornecedor/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `checklist_forncedor${new Date().toISOString().slice(0,10)}.xls`);
