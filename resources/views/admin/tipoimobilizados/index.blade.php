<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Tipo de Imobilizado') }}
            </h2>
            <div class="flex items-center space-x-4">
                <a href="{{ route('admin.tipoimobilizados.create') }}"
                ...
                <div class="relative inline-block text-left" x-data="{ helpOpen: false }">
                    ...
                    <div x-show="helpOpen" @click.away="helpOpen = false"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                <p class="text-sm leading-5 font-medium text-gray-900 truncate">Ajuda - Tipo Imobilizado
                                </p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode visualizar todos os tipos de imobilizados cadastrados. Utilize
                                    o botão 'Cadastrar Tipo' para adicionar um novo registro. Você pode editar ou
                                    excluir tipos existentes utilizando as ações disponíveis em cada linha da tabela.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-4 bg-white border-b border-gray-200">
            ...
            <!-- Search Form -->
            @include('admin.tipoimobilizados._search-form')

            <div class="overflow-x-auto relative min-h-[400px]">
                <!-- Loading indicator -->


                @include('admin.tipoimobilizados._table')
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Função para editar tipo de imobilizado
        function editarTipoImobilizado(id) {
            window.location.href = `{{ route('admin.tipoimobilizados.edit', ':id') }}`.replace(':id', id);
        }
        ...
        function executarExclusao() {
            ...
            fetch(`{{ route('admin.tipoimobilizados.destroy', ':id') }}`.replace(':id', idExclusao), {
                method: 'DELETE',
                headers: {
            ...
            .catch(error => {
                ...
                showNotification(
                    'Erro',
                    error.message || 'Não foi possível excluir o tipo de imobilizado',
                    'error'
                );
            })
            .finally(() => {
