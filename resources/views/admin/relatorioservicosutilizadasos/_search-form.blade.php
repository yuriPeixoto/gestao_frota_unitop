<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_servico" label="Serviços:" placeholder="Selecionar" :options="$servico"
            value="{{ request('id_servico') }}" :searchUrl="route('admin.api.servico.search')" asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioServicosUtilizadasOs.gerarPdf()"
                :disabled="$store.relatorioServicosUtilizadasOs.loading"
                ...
                <span x-show="$store.relatorioServicosUtilizadasOs.loading" class="loading-spinner mr-2"></span>
                <!-- Ícone normal (quando não carregando) -->
                <x-icons.magnifying-glass x-show="!$store.relatorioServicosUtilizadasOs.loading" class="h-4 w-4 mr-2" />

                <!-- Texto do botão -->
                <span x-text="$store.relatorioServicosUtilizadasOs.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioServicosUtilizadasOs.gerarExcel()"
                :disabled="$store.relatorioServicosUtilizadasOs.loading"
                ...
                <!-- Texto do botão -->
                <span x-text="$store.relatorioServicosUtilizadasOs.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
        </div>
