<form method="GET" action="{{ route('admin.manutencaopneusentrada.index') }}" class="space-y-4"
    hx-get="{{ route('admin.manutencaopneusentrada.index') }}" hx-target="#results-table" hx-select="#results-table"
    hx-trigger="change delay:500ms, search">

    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">

        <x-forms.input name="id_pneu" label="Cód. Os Manutenção Pneu" value="{{ request('id_pneu') }}" />
        ...
        <x-forms.smart-select name="id_filial" label="Filial" placeholder="Selecione o modelo..." :options="$formOptions['filiais']"
