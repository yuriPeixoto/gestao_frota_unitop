<div class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            {{-- <button type="button" x-on:click="$store.relatorioconferenciatabelao.gerarPdf()"
                :disabled="$store.relatorioconferenciatabelao.loading"
                ...
                <span x-show="$store.relatorioconferenciatabelao.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioconferenciatabelao.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioconferenciatabelao.loading ? 'Gerando...' : 'Conferêncial PDF'"></span>
            </button> --}}

            <button type="button" x-on:click="$store.relatorioconferenciatabelao.gerarExcel()"
                :disabled="$store.relatorioconferenciatabelao.loading"
                ...
                <span x-show="$store.relatorioconferenciatabelao.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioconferenciatabelao.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioconferenciatabelao.loading ? 'Gerando...' : 'Tabelão'"></span>
            </button>
