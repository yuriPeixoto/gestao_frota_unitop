/**
 * Script para gerenciamento de documentos de sinistros
 * Versão otimizada com upload AJAX e feedback visual
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    feedbackContainer.className = 'hidden fixed top-4 right-4 p-4 rounded shadow-lg z-50';
    feedbackContainer.style.minWidth = '250px';
    ...
    function adicionarDocumento() {
        ...
        if (file.size > 10 * 1024 * 1024) {
            mostrarFeedback('O arquivo é muito grande. Tamanho máximo: 10MB', 'error');
            return;
        }
        ...
        if (!extensoesPermitidas.includes(extensao)) {
            mostrarFeedback('Formato de arquivo não permitido. Use: PDF, imagens ou documentos Office.', 'error');
            return;
        }
        ...
        // Iniciar upload com feedback
        uploadInProgress = true;
        mostrarFeedback(`Enviando ${fileName}...`, 'info', true);

        // Enviar o arquivo ao servidor via AJAX
        fetch('/admin/sinistros/documentos/upload', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        registrosDocTemporarios.forEach((registro, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    ${docName}
                    <a href="#" class="text-blue-500 ml-2" title="Visualizar" onclick="visualizarDocumento('${registro.doc}'); return false;">
    ...
    function excluirDocRegistro(index) {
        ...
        // Se for um documento temporário (upload recente), tenta remover do servidor
        if (documento.doc && !documento.doc.includes('sinistros/')) {
            // Verificar se o documento está na pasta temp
            fetch('/admin/sinistros/documentos/excluir', {
                method: 'DELETE',
                headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        console.warn('Não foi possível remover o arquivo temporário');
                    }
                })
                .catch(error => {
    ...
    function visualizarDocumento(docPath) {
        // Determinar o tipo de documento (temporário ou permanente)
        let url;

        if (docPath.startsWith('sinistros/')) {
            // Documento já salvo
            url = `/admin/sinistros/documentos/arquivo/${btoa(docPath)}`;
        } else {
            // Documento temporário
            url = `/admin/sinistros/documentos/arquivo/${btoa('temp/' + docPath)}`;
        }
    ...
    function mostrarFeedback(mensagem, tipo, persistente = false) {
        ...
        feedbackContainer.className = `fixed top-4 right-4 p-4 rounded shadow-lg z-50 flex items-center ${bgColor} ${textColor}`;
        feedbackContainer.innerHTML = `${icon} <span>${mensagem}</span>`;

        // Mostrar o feedback
        feedbackContainer.classList.remove('hidden');

        // Remover após um tempo, a menos que seja persistente
        if (!persistente) {
            setTimeout(() => {
    ...
    // Adicionar listener ao botão de adicionar documento
    const addDocumentoBtn = document.querySelector('button[onclick="adicionarDocumento()"]');
    if (addDocumentoBtn) {
        // Remover o handler inline e adicionar o listener
    ...
    // Verificar se há um campo de upload e adicionar eventos
    const documentoInput = document.querySelector('[name="documento"]');
    if (documentoInput) {
        // Adicionar feedback visual ao selecionar arquivo
