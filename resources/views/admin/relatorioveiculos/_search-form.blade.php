<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_tipo_equipamento" label="Tipo Equipamento:" placeholder="Selecionar"
            :options="$equipamento" value="{{ request('id_tipo_equipamento') }}" />
        ...
        <x-forms.smart-select name="situacao_veiculo" label="Ativo:" placeholder="Selecionar" :options="$ativo"
            value="{{ request('situacao_veiculo') }}" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioveiculos.gerarPdf()"
                :disabled="$store.relatorioveiculos.loading"
                ...
                <span x-show="$store.relatorioveiculos.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioveiculos.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioveiculos.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioveiculos.gerarExcel()"
                :disabled="$store.relatorioveiculos.loading"
                ...
                <span x-show="$store.relatorioveiculos.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioveiculos.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioveiculos.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
