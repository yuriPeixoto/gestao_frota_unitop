<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="status_pneu" label="Status do Pneu:" placeholder="Selecionar" :options="$status"
            value="{{ request('status_pneu') }}" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriopneusstatus.gerarPdf()"
                :disabled="$store.relatoriopneusstatus.loading"
                ...
                <span x-show="$store.relatoriopneusstatus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopneusstatus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopneusstatus.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriopneusstatus.gerarExcel()"
                :disabled="$store.relatoriopneusstatus.loading"
                ...
                <span x-show="$store.relatoriopneusstatus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopneusstatus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopneusstatus.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
