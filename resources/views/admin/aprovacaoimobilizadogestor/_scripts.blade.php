...
<script>
    ...
    window.onAprovar = function(id) {

        fetch(`{{ route('admin.aprovacaoimobilizadogestor.aprovar') }}`, {
            method: 'POST',
            headers: {
    ...
    // Coloque isso em um arquivo .js ou dentro de uma tag <script> no seu blade
    window.onReprovar = function(id) {
        const  observacao_gestor = document.getElementById('observacao_gestor')

        fetch(`{{ route('admin.aprovacaoimobilizadogestor.reprovar') }}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
            ...
            body: JSON.stringify({ id: id, observacao_gestor: observacao_gestor.value })
        })
        .then(response => {
            if (!response.ok) {
                return response.json().then(err => { throw err; });
