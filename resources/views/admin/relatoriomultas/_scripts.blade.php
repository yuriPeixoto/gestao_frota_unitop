            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomultas/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `motivo_multa${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriomultas/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `motivo_multa${new Date().toISOString().slice(0,10)}.xls`);
