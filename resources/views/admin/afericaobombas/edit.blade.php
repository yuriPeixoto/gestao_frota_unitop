<x-app-layout>
            ...
            <div class="flex items-center space-x-4">
                <div class="relative inline-block text-left" x-data="{ helpOpen: false }">
                    ...
                    <div x-show="helpOpen" @click.away="helpOpen = false"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                <p class="text-sm leading-5 font-medium text-gray-900 truncate">Ajuda - Gerar Entrada
                                    por Aferição</p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode gerar uma entrada a partir de uma aferição existente.
                                    Verifique os dados da aferição, selecione o tanque e indique o volume de entrada.
                                    O volume de entrada não pode ser maior que o volume de abastecimento.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <form action="{{ route('admin.afericaobombas.update', $afericao->id_abastecimento_integracao) }}" method="POST"
        class="py-4">
        @csrf
        @method('PUT')
                    ...
                    <div x-data="afericaoForm()">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="volume" class="block text-sm font-medium text-gray-700">Volume
                                        Abastecimento</label>
                                    <input type="text" id="volume" name="volume" readonly
                                        class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                        value="{{ number_format($afericao->volume, 2, ',', '.') }}">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="data_inicio" name="data_inicio" readonly
                                        class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                        value="{{ is_object($afericao->data_inicio) ? $afericao->data_inicio->format('d/m/Y H:i') : (is_string($afericao->data_inicio) && $afericao->data_inicio ? date('d/m/Y H:i', strtotime($afericao->data_inicio)) : '') }}">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select id="id_tanque" name="id_tanque" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm {{ $tanqueSelecionado ? 'bg-gray-100' : '' }}"
                                        {{ $tanqueSelecionado ? 'readonly' : '' }}>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                                <div>
                                    ...
                                    <input type="number" id="volume_entrada" name="volume_entrada" step="0.01" required
                                        ...
                                        value="{{ old('volume_entrada', $afericao->volume) }}"
        ...
        function afericaoForm() {
                return {
                    ...
                    validarVolume() {
                        ...
                        const volumeAbastecimento = parseFloat('{{ $afericao->volume }}');
            ...
            // Tornar o select de tanque readonly quando tiver valor selecionado
            document.addEventListener('DOMContentLoaded', function() {
                const tanqueSelect = document.getElementById('id_tanque');
                if (tanqueSelect && tanqueSelect.classList.contains('bg-gray-100')) {
                    tanqueSelect.setAttribute('disabled', true);
                    // Garantir que o valor será enviado mesmo com o select desabilitado
                    const form = tanqueSelect.closest('form');
                    form.addEventListener('submit', function() {
                        const hiddenInput = document.createElement('input');
                        hiddenInput.type = 'hidden';
                        hiddenInput.name = 'id_tanque';
                        hiddenInput.value = tanqueSelect.value;
