<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioChecklist', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriochecklist/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_tabela_checklist_${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriochecklist/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_tabela_checklist_${new Date().toISOString().slice(0,10)}.xls`);
