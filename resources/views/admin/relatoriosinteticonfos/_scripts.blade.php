<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioSinteticoNfOs', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosinteticonfos/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `notas_fiscais_detalhado${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriosinteticonfos/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `notas_fiscais_detalhado_v2${new Date().toISOString().slice(0,10)}.xls`);
