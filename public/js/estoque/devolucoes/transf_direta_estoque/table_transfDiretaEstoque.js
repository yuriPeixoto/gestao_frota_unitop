window.onload = function () {
    ...
    const transfDiretaEstoqueJson = document.getElementById('devTransfDiretaEstoque_json').value;
    const transfDiretaEstoques = JSON.parse(transfDiretaEstoqueJson);


    if (transfDiretaEstoques && transfDiretaEstoques.length > 0) {
        transfDiretaEstoques.forEach(transfDiretaEstoque => {
            registrosTemporarios.push({
                id_produto: transfDiretaEstoque.id_produto,
                descricao_produto: transfDiretaEstoque.produto.descricao_produto,
                unidade_produto: transfDiretaEstoque.produto.unidade_produto.descricao_unidade,
                id_unidade_produto: transfDiretaEstoque.produto.id_unidade_produto,
                qtde_produto: transfDiretaEstoque.qtde_produto,
                qtde_devolucao: transfDiretaEstoque.qtde_devolucao || 0
            });
        });
        atualizarTabela();
    }

    function adicionartransfDiretaEstoque() {
        ...
        document.getElementById('devTransfDiretaEstoque_json').value = JSON.stringify(registrosTemporarios);
    }

    function atualizarTabela() {
        const tbody = document.getElementById('tabelaTransfDiretaEstoqueBody');
        if (!tbody) {
            console.error('Elemento #tabelaTransfDiretaEstoqueBody não encontrado');
            return;
        }
    ...
    window.adicionartransfDiretaEstoque = adicionartransfDiretaEstoque;
