<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriorecebimentocombustivel', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tanque: document.querySelector('select[name="id_tanque"], #id_tanque')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriorecebimentocombustivel/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_recebimento_combustivel${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriorecebimentocombustivel/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_recebimento_combustivel${new Date().toISOString().slice(0,10)}.xls`);
