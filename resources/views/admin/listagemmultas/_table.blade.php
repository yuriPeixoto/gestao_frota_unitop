...
<div class="results-table">
    <x-tables.table>
        ...
        <x-tables.body>
            @forelse ($listagemMultas as $index => $listagemMulta)
            <x-tables.row :index="$index">
                <x-tables.cell class="relative">
                    @php $dropdownId = 'dropdown-' . $listagemMulta->ait; @endphp
                    ...
                    <div class="relative inline-block text-left">
                        ...
                        <div id="{{ $dropdownId }}"
                            ...
                            <ul class="py-1">
                                <li>
                                    <a onclick="handleDropdownAction('{{ $dropdownId }}', () => abrirCondutorForm({{ $listagemMulta }}))"
                                ...
                                <li>
                                    <a onclick="handleDropdownAction('{{ $dropdownId }}', () => abrirModalPdf({{ $listagemMulta }}, 'notificacao'))"
                                        class="block px-4 py-3 text-gray-700 hover:bg-gray-100 flex items-center">
                                ...
                                <li>
                                    <a onclick="handleDropdownAction('{{ $dropdownId }}', () => removerMotorista('{{ $listagemMulta->ait }}'))"
                                        class="block px-4 py-3 text-gray-700 hover:bg-gray-100 flex items-center">
                                ...
                                <li>
                                    <a href="#"
                                        onclick="gerarFici(event, '{{ $listagemMulta->id_smartec_multas_sne_detran }}')"
                                        class="block px-4 py-3 text-gray-700 hover:bg-gray-100 flex items-center">
                                        <x-icons.print class="w-4 h-4 mr-3 text-blue-600" />
                                        Gerar FICI
                                    </a>
                ...
                <x-tables.cell>
                    @if ($listagemMulta->confirmacao_pagamento == 'true')
                    Sim
                    @else
                    Não
                    @endif
                </x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->placa }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->prazo_indicacao }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->motorista_nome }}</x-tables.cell>
                <x-tables.cell>{{ format_date($listagemMulta->data_infracao, 'd/m/Y') }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->ait }}</x-tables.cell>
                <x-tables.cell>{{$listagemMulta->ait_originaria }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->orgao_autuador }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->descricao }}</x-tables.cell>
                <x-tables.cell>R$ {{ number_format($listagemMulta->valor_a_pagar, 2, ',', '.') }}</x-tables.cell>
                <x-tables.cell>{{ format_date($listagemMulta->boleto_vencimento, 'd/m/Y') }}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->local }}</x-tables.cell>
                <x-tables.cell>{{$listagemMulta->gravidade}}</x-tables.cell>
                <x-tables.cell>{{ $listagemMulta->confirmacao_pagamento_manual }}</x-tables.cell>
                ...
                <x-bladewind.modal name="solicitarDesconto" cancel_button_label="" ok_button_label=""
                    ...
                    <form id="descontoForm" method="POST">
                        @csrf
                        <input type="hidden" id="multaId" name="id_smartec_multas_sne_detran" value="">
