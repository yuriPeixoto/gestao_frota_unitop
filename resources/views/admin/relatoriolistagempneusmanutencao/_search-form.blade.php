<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <!-- Filial -->
        <x-forms.smart-select name="id_tipo_reforma" label="Tipo Reforma:" placeholder="Selecionar"
            :options="$tiporeforma" value="{{ request('id_tipo_reforma') }}" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriolistagempneusmanutencao.gerarPdf()"
                :disabled="$store.relatoriolistagempneusmanutencao.loading"
                ...
                <span x-show="$store.relatoriolistagempneusmanutencao.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriolistagempneusmanutencao.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriolistagempneusmanutencao.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriolistagempneusmanutencao.gerarExcel()"
                :disabled="$store.relatoriolistagempneusmanutencao.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatoriolistagempneusmanutencao.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriolistagempneusmanutencao.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
