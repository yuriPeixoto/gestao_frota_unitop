<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioNotaFiscalExterna', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorionotafiscalexterna/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_nota_fiscal_externa_${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorionotafiscalexterna/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_nota_fiscal_externa_${new Date().toISOString().slice(0,10)}.xls`);
