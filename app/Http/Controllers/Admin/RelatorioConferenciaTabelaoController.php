...
class RelatorioConferenciaTabelaoController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.relatorioconferenciatabelao.index', compact('motorista', 'placa'));
    }
    ...
    public function gerarExcel(Request $request)
    {
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            ...
            if (empty($request['descricao_tipo'])) {
                $in_equipamento  = '!=';
                $id_equipamento  = '0';
            } else {
                $in_equipamento  = '=';
                $id_equipamento  = $request['descricao_tipo'];
            }
            ...
            if (empty($request['id_mot_unitop'])) {
                ...
                $in_pessoal     = '=';
                $id_pessoal     = $request['id_mot_unitop'];
            }
            ...
            $parametros = array(
                ...
                'P_in_veiculo'      => $in_equipamento,
                'P_id_veiculo'    => $id_equipamento,
                'P_in_motorista'    => $in_pessoal,
                'P_id_motorista'    => $id_pessoal
            );

            $name       = 'historico_premiacao_tabelao_v4';
