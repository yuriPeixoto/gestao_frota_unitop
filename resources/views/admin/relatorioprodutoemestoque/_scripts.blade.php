<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioprodutoemestoque', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoemestoque/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_produtos_em_estoque${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioprodutoemestoque/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_produtos_em_estoque${new Date().toISOString().slice(0,10)}.xls`);
