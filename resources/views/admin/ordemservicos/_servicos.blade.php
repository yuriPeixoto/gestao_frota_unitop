...
<div id="alertaEdicaoServico" class="hidden bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
    <div class="flex">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
    ...
    {{-- Serviço --}}
    <div>
        <x-forms.smart-select name="id_servicos" id="id_servicos" label="Serviço" placeholder="Selecione o serviço..."
            :options="[]" :searchUrl="route('admin.ordemservicos.getServicosSearch', ['origem' => 6])" :selected="old('id_servico', $ordemServico->id_servico ?? '')" asyncSearch="true" />
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-2 mt-4">
    <div>
        ...
        <input type="number" id="servico_quantidade" name="servico_quantidade" step="1" min="1"
    ...
    <div>
        ...
        <input type="text" id="servico_valor" name="servico_valor" readonly
            class="relative w-full flex items-center border bg-gray-100 border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-[10px] text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
    </div>
    <div>
        {{-- total_desconto --}}
        ...
        <input type="text" id="valor_descontoservico" name="valor_descontoservico" readonly
            class="resultado mt-1 block bg-gray-100 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
        ...
        <table class="min-w-full divide-y divide-gray-200 shadow-md overflow-hidden sm:rounded-md tabelaServicosBody">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                <tr>
                    ...
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Código do Serviço</th>
                    ...
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Quantidade de Serviço</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Serviços</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Valor Serviço</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Valor Desconto Serviço</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Valor Total com Desconto</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Serviço Finalizado?</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Nota Fiscal de Serviço</th>
                    <th scope="col" class="py-3 px-2 whitespace-nowrap">Status Serviço</th>
                </tr>
            </thead>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(function() {
            const produtoSelect = document.querySelector('input[name="id_servicos"]');
            if (produtoSelect) {
                ...
                if (alpineElement && alpineElement._x_dataStack && alpineElement._x_dataStack[0]) {
                    ...
                    component.searchItems = async function() {
                        ...
                        if (typeof operacaoSelecionada !== 'undefined') {
                            const baseUrl =
                                '{{ route('admin.ordemservicos.getServicosSearch', ['operacao' => 'PLACEHOLDER']) }}';
