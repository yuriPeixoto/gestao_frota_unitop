<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriopneusestoque', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_controle_vida_pneu: document.querySelector('select[name="iid_controle_vida_pneu"], #iid_controle_vida_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusestoque/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_estoque${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusestoque/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_estoque_v2${new Date().toISOString().slice(0,10)}.xls`);
