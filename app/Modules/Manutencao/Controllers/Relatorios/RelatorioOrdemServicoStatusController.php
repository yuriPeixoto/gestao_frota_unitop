...
class RelatorioOrdemServicoStatusController extends Controller
{
    public  function index(Request $request)
    {
        ...
        $status = StatusOrdemServico::select('id_status_ordem_servico as value', 'situacao_ordem_servico as label')
            ->orderBy('id_status_ordem_servico')
            ->limit(30)
            ->get();

        return view('admin.relatorioordemservicostatus.index', [
            'filial' => $filial,
            'status' => $status,
        ]);
    }
    ...
    public function gerarPdf(Request $request)
    {
        ...
        $pecas  = empty($request['id_status_ordem_servico']) ? "0" : $request['id_status_ordem_servico'];
        $filial = empty($request['id']) ? "0" : $request['id'];

        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            if (empty($request['id_status_ordem_servico'])) {
                $in_pecas  = '!=';
                $id_pecas  = '0';
            } else {
                $in_pecas  = 'IN';
                $id_pecas  = implode(",", $pecas);
            }
            if (empty($request['id'])) {
                $in_filial  = '!=';
                $id_filial  = '0';
            ...
            $parametros = array(
                ...
                'P_in_status'       => $in_pecas,
                'P_id_status'   => $id_pecas
            );


            $name       = 'ordem_servico_por_status_v1';
            $agora      = date('d-m-YH:i');
            $tipo       = '.pdf';
    ...
    public function gerarExcel(Request $request)
    {
        ...
        $param = [
            ...
            'id_status_ordem_servico' => $request->id_status_ordem_servico,
        ];

        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) { //sempre verificar a data
            if (empty($request['id_status_ordem_servico'])) {
                $in_pecas  = '!=';
                $id_pecas  = '0';
            } else {
                $in_pecas  = 'IN';
                $id_pecas  = $param('id_status_ordem_servico');
            }
            ...
            $name = 'ordem_servico_por_status_v2';
