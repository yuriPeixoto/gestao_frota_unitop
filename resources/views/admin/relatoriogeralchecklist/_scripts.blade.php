<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioGeralChecklist', {
            loading: false,

            //Aqui colcamos os parâmetros de acordo com o html
            coletarDadosFormulario() {
                return {
                    data_realizacao: document.querySelector('[name="data_realizacao"]')?.value || '',
                    ...
                    id_veiculo: document.querySelector('select[name="id_veiculo"], #id_veiculo')?.value || '',
                    idobtermotorista: document.querySelector('select[name="idobtermotorista"], #idobtermotorista')?.value || '',
                    tipo: document.querySelector('select[name="tipo"], #tipo')?.value || '',
                };
            },

            //verificação de datas
            validarDatas(data) {
                if (!data.data_realizacao || !data.data_final) {
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriogeralchecklist/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `checklist_geral${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriogeralchecklist/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_geral_checklist_${new Date().toISOString().slice(0,10)}.xls`);
