<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriofichacontroleestoque', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriofichacontroleestoque/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `controle_de_estoque${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriofichacontroleestoque/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `controle_de_estoque${new Date().toISOString().slice(0,10)}.xls`);
