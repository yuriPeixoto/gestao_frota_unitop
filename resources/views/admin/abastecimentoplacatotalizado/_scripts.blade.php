<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('utils', {
            ...
            async _fazerRequisicao(url, data, timeout = 300000, acceptHeader = 'application/json') {
            ...
            // Método para processar resposta e verificar tipo de conteúdo
            async _processarResposta(response, tiposEsperados = []) {
                ...
                // Verificar se o tipo de conteúdo é esperado
                const tipoValido = tiposEsperados.some(tipo => contentType && contentType.includes(tipo));
                if (!tipoValido && tiposEsperados.length > 0) {
            ...
            async imprimirAbastecimentoPlaca() {
                this.loading = true;
                ...
                try {
                    ...
                    const response = await this._fazerRequisicao(
                        '/admin/abastecimentoplacatotalizado/imprimir', 
                        data,
                        300000, // 5 minutos
                        'application/pdf, application/json'
                    );
                    
                    const blob = await this._processarResposta(response, ['application/pdf']);
                    
                    this._fazerDownload(blob, `abastecimento_placa_totalizado_${new Date().toISOString().split('T')[0]}.pdf`);
            ...
            async imprimirAbastecimentoPlacaTotalizadoExcel() {
                this.loading = true;
                ...
                try {
                    ...
                    const response = await this._fazerRequisicao(
                        '/admin/abastecimentoplacatotalizado/imprimirexcel', 
                        data,
                        300000, // 5 minutos
                        'application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/json'
                    );
                    
                    const blob = await this._processarResposta(response, [
                        'application/vnd.ms-excel',
                        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                        'application/octet-stream'
                    ]);
                    
                    this._fazerDownload(blob, `relatorio_abastecimento_placa_totalizado_${new Date().toISOString().split('T')[0]}.xls`);
