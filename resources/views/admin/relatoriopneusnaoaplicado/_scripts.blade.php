<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriopneusnaoaplicado', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusnaoaplicado/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_nao_Aplicados${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusnaoaplicado/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `Pneus_nao_Aplicados${new Date().toISOString().slice(0,10)}.xls`);
