<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriodataentregapedidos', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriodataentregapedidos/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `data_entrega${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriodataentregapedidos/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `data_entrega${new Date().toISOString().slice(0,10)}.xls`);
