...
<script>
    function abrirModal(id, placa) {
        console.log('abrirModal', id, placa);
    ...
    function onVerificarSituacao(id, status) {

        if (status == 5) {
            if (!validarAprovacao(id)) {
                return; // Se a validação falhar, não prossegue com a requisição
            }
        } 
    ...
    function onEmitirOrdemServico(id) {
        let formData = new FormData();

        formData.append('id', id);

        fetch(`{{ route('admin.devolucaoimobilizadoveiculo.emitirordemservico') }}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
    ...
    function onEmitirSinistro(id) {
        let formData = new FormData();

        formData.append('id', id);

        fetch(`{{ route('admin.devolucaoimobilizadoveiculo.emitirsinistro') }}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
    ...
    function validarAprovacao(id) {
        const checklist = document.getElementById('checklist-' + id);
        const documento = document.getElementById('documento-' + id);
        
        if (!checklist.checked) {
            alert('Por favor, confirme que o checklist foi verificado.');
            return false;
        }
        
        // Só valida documento se existir (tipo COMODATO)
        if (documento && !documento.checked) {
            alert('Por favor, confirme que a documentação foi verificada.');
            return false;
        }
