<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_nota_fiscal" label="N° da Nota Fiscal:" placeholder="Selecionar"
            :options="$notafiscal" value="{{ request('id_nota_fiscal') }}"
            :searchUrl="route('admin.api.notasfiscais.search')" asyncSearch="false" />

        <x-forms.smart-select name="id_fornecedor" label="Fornecedor:" placeholder="Selecionar" :options="$fornecedor"
            value="{{ request('id_fornecedor') }}" :searchUrl="route('admin.api.fornecedores.search')"
            asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioentradanotasfiscais.gerarPdf()"
                :disabled="$store.relatorioentradanotasfiscais.loading"
                ...
                <span x-show="$store.relatorioentradanotasfiscais.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioentradanotasfiscais.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioentradanotasfiscais.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioentradanotasfiscais.gerarExcel()"
                :disabled="$store.relatorioentradanotasfiscais.loading"
                ...
                <span x-show="$store.relatorioentradanotasfiscais.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioentradanotasfiscais.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioentradanotasfiscais.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
