<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriohistoricoimobilizado', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriohistoricoimobilizado/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `movimentacao_imobilizados${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriohistoricoimobilizado/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `movimentacao_imobilizados_v2${new Date().toISOString().slice(0,10)}.xls`);
