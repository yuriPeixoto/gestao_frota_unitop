<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioquantidadepneusporfilial', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioquantidadepneusporfilial/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `quantidade_pneu_filial${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioquantidadepneusporfilial/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `quantidade_pneu_filial${new Date().toISOString().slice(0,10)}.xls`);
