<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioentradadepneumanutencao', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_vida_pneu: document.querySelector('select[name="id_vida_pneu"], #id_vida_pneu')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioentradadepneumanutencao/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `entrada_pneu_manutencao_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioentradadepneumanutencao/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `entrada_pneu_manutencao_v2${new Date().toISOString().slice(0,10)}.xls`);
