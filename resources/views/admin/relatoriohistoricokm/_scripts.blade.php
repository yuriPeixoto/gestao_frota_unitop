<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriohistoricokm', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriohistoricokm/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `historico_km${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriohistoricokm/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `historico_km${new Date().toISOString().slice(0,10)}.xls`);
