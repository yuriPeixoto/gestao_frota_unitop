<form method="GET" action="{{ route('admin.requisicaopneusvendassaida.index') }}" class="space-y-4"
    hx-get="{{ route('admin.requisicaopneusvendassaida.index') }}" hx-target="#results-table" hx-select="#results-table"
    hx-trigger="change delay:500ms, search">

    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
        <x-forms.input name="id_requisicao_pneu" label="Código Requisição" value="{{ request('id_requisicao_pneu') }}" />
        ...
        <x-forms.smart-select name="id_usuario" label="Usuario solicitante"
            placeholder="Selecione o usuario solicitante..." :options="$form['pessoa']" :selected="request('id_usuario')" asyncSearch="true" />
    </div>
    ...
    <div class="flex justify-between mt-4">
        ...
        <div>
            ...
            <x-ui.export-buttons route="admin.requisicaopneusvendassaida" :formats="['pdf', 'csv', 'xls', 'xml']" />
        </div>
    </div>
</form>
