<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <!-- Filial -->
        <x-forms.smart-select name="id_tipo_descarte" label="Tipo Descarte:" placeholder="Selecionar"
            :options="$tipodescarte" value="{{ request('id_tipo_descarte') }}" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriolistagempneusdescartados.gerarPdf()"
                :disabled="$store.relatoriolistagempneusdescartados.loading"
                ...
                <span x-show="$store.relatoriolistagempneusdescartados.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriolistagempneusdescartados.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriolistagempneusdescartados.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriolistagempneusdescartados.gerarExcel()"
                :disabled="$store.relatoriolistagempneusdescartados.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatoriolistagempneusdescartados.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriolistagempneusdescartados.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
