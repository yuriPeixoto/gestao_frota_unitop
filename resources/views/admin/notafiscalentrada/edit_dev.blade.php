    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Devolução Nota Fiscal #') }}{{ $nfEntrada->id_nota_fiscal_entrada }}
            </h2>
            <x-help-icon title="Ajuda - Devolução Nota Fiscal de Entrada"
                content="Nesta tela você pode editar a devolução da nota fiscal selecionada. Preencha os campos e clique em salvar." />
        </div>
    </x-slot>
    ...
    <div>
        <div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.notafiscalentrada.devolve', $nfEntrada) }}" method="POST"
                        x-data="{ isSubmitting: false }"
                        @submit.prevent="if (!isSubmitting) { isSubmitting = true; $el.submit(); }">
                        @csrf
                        @method('PUT')
                        @include('admin.notafiscalentrada._form_dev')
