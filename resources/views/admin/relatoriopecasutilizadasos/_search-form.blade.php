<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_produto" label="Peças:" placeholder="Selecionar" :options="$pecas"
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioPecasUtilizadas.gerarPdf()"
                :disabled="$store.relatorioPecasUtilizadas.loading"
                ...
                <span x-show="$store.relatorioPecasUtilizadas.loading" class="loading-spinner mr-2"></span>
                <!-- Ícone normal (quando não carregando) -->
                <x-icons.magnifying-glass x-show="!$store.relatorioPecasUtilizadas.loading" class="h-4 w-4 mr-2" />

                <!-- Texto do botão -->
                <span x-text="$store.relatorioPecasUtilizadas.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioPecasUtilizadas.gerarExcel()"
                :disabled="$store.relatorioPecasUtilizadas.loading"
                ...
                <!-- Texto do botão -->
                <span x-text="$store.relatorioPecasUtilizadas.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
        </div>
