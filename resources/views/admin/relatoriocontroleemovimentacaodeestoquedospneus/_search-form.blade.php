<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_pneu" label="Fogo:" placeholder="Selecionar" :options="$pneus"
        ...
        <x-forms.smart-select name="id_modelo_pneu" label="Modelo Pneu:" placeholder="Selecionar" :options="$modelo"
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriocontroleemovimentacaodeestoquedospneus.gerarPdf()"
                :disabled="$store.relatoriocontroleemovimentacaodeestoquedospneus.loading"
                ...
                <span x-show="$store.relatoriocontroleemovimentacaodeestoquedospneus.loading"
                    class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriocontroleemovimentacaodeestoquedospneus.loading"
                    class="h-4 w-4 mr-2" />
                <span
                    x-text="$store.relatoriocontroleemovimentacaodeestoquedospneus.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriocontroleemovimentacaodeestoquedospneus.gerarExcel()"
                :disabled="$store.relatoriocontroleemovimentacaodeestoquedospneus.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatoriocontroleemovimentacaodeestoquedospneus.loading"
                    class="h-4 w-4 mr-2" />
                <span
                    x-text="$store.relatoriocontroleemovimentacaodeestoquedospneus.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
