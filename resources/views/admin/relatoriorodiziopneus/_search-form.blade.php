<div class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            {{-- <button type="button" x-on:click="$store.relatoriorodiziopneus.gerarPdf()"
                :disabled="$store.relatoriorodiziopneus.loading"
                ...
                <span x-show="$store.relatoriorodiziopneus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriorodiziopneus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriorodiziopneus.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button> --}}

            <button type="button" x-on:click="$store.relatoriorodiziopneus.gerarExcel()"
                :disabled="$store.relatoriorodiziopneus.loading"
                ...
                <span x-show="$store.relatoriorodiziopneus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriorodiziopneus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriorodiziopneus.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
