<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioipvalicenciamento', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    data_base_vencimento: document.querySelector('select[name="data_base_vencimento"], #data_base_vencimento')?.value || '',
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioipvalicenciamento/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `venc_ipva_licen${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioipvalicenciamento/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `venc_ipva_licen${new Date().toISOString().slice(0,10)}.xls`);
