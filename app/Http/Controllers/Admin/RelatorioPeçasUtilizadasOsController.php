...
class RelatorioPeçasUtilizadasOsController extends Controller
{
    public  function index(Request $request)
    {
        $query = OrdemServicoPecas::with([
            'id_produto',
        ]);
        ...
        $pecas = Produto::select('id_produto as value', 'descricao_produto as label')
            ->orderBy('descricao_produto')
            ->limit(30)
            ->get();


        return view('admin.relatoriopecasutilizadasos.index', [
            'filial' => $filial,
            'pecas' => $pecas,
        ]);
    }
    ...
    public function gerarPdf(Request $request)
    {
        ...
        $param = [
            ...
            'id_produto' => $request->id_produto, // <-- CORRIGIDO
        ];
        ...
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            if (empty($request['id_produto'])) {
                ...
                $in_pecas  = 'IN';
                $id_pecas  = $param['id_produto']; // <-- CORRIGIDO
            }
    ...
    public function gerarExcel(Request $request)
    {
        ...
        if (!empty($request['data_inclusao']) && !empty($request['data_final'])) {
            ...
            $name = 'pecas_utilizadas_por_os_v2';
            $agora = now()->format('d-m-Y_H-i');
