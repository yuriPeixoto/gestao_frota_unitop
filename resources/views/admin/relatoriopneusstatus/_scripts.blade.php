<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriopneusstatus', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusstatus/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_por_status_v2${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                //    if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriopneusstatus/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `pneus_por_status_v2${new Date().toISOString().slice(0,10)}.xls`);
