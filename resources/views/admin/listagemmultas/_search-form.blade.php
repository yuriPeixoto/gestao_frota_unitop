<form method="GET" action="{{ route('admin.listagemmultas.index') }}" class="space-y-4"
    hx-get="{{ route('admin.listagemmultas.index') }}" hx-target="#results-table" hx-select="#results-table"
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div>
            <x-forms.smart-select id="placa" name="placa[]" label="Placa" placeholder="Selecione a(s) placa(s)..."
                :options="$veiculos" :multiple="true" asyncSearch="false" />
        </div>
    ...
    <div class="flex justify-between mt-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <x-ui.export-buttons route="admin.listagemmultas" :formats="['pdf', 'csv', 'xls', 'xml']" />
