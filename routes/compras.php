...
use App\Modules\Compras\Controllers\Admin\AprovarPedidoController;
use App\Modules\Compras\Controllers\Admin\ContratoFornecedorController;
use App\Modules\Compras\Controllers\Admin\CotacoesController;
...
use App\Modules\Compras\Controllers\Admin\ItemSolicitacaoCompraController;
use App\Modules\Compras\Controllers\Admin\OrcamentoController;
use App\Modules\Compras\Controllers\Admin\PedidoCompraController;
use App\Modules\Compras\Controllers\Admin\SolicitacaoCompraController;
use App\Modules\Compras\Controllers\Admin\NotaFiscalAvulsaController;
...
Route::group(['prefix' => 'compras', 'as' => 'compras.'], function () {
    ...
    Route::get('/', function () {
        ...
        $solicitacoesPendentes = \App\Modules\Compras\Models\SolicitacaoCompra::pendentes()->count();
        $pedidosPendentes = \App\Modules\Compras\Models\PedidoCompra::pendentesAprovacao()->count();
        $pedidosAprovados = \App\Modules\Compras\Models\PedidoCompra::aprovados()->count();
        $valorTotalMes = \App\Modules\Compras\Models\PedidoCompra::where('data_inclusao', '>=', now()->startOfMonth())
            ->where('situacao_pedido', '!=', 6)
            ->sum('valor_total');

        $pedidosRecentes = \App\Modules\Compras\Models\PedidoCompra::with(['fornecedor', 'comprador'])
            ->orderBy('data_inclusao', 'desc')
            ->limit(5)
            ->get();

        $meusPedidos = \App\Modules\Compras\Models\PedidoCompra::with(['fornecedor'])
            ->where('id_comprador', Auth::id())
            ->orderBy('data_inclusao', 'desc')
            ->limit(5)
            ->get();

        $pedidosPendentesAprovacao = \App\Modules\Compras\Models\PedidoCompra::with(['fornecedor', 'solicitacaoCompra.solicitante'])
            ->pendentesAprovacao()
            ->limit(5)
            ->get();

        $ultimasAtividades = \App\Modules\Compras\Models\PedidoCompra::with(['comprador'])
            ->orderBy('data_inclusao', 'desc')
            ->limit(10)
            ->get()
            ->map(function ($pedido) {
