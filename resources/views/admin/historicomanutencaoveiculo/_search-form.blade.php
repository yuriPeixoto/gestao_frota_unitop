<div class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.historicoManuntencao.gerarPdf()"
                :disabled="$store.historicoManuntencao.loading"
                ...
                <span x-show="$store.historicoManuntencao.loading" class="loading-spinner mr-2"></span>
                <!-- Ícone normal (quando não carregando) -->
                <x-icons.magnifying-glass x-show="!$store.historicoManuntencao.loading" class="h-4 w-4 mr-2" />

                <!-- Texto do botão -->
                <span x-text="$store.historicoManuntencao.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.historicoManuntencao.gerarExcel()"
                :disabled="$store.historicoManuntencao.loading"
                ...
                <!-- Texto do botão -->
                <span x-text="$store.historicoManuntencao.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
        </div>
