<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_nf_ordem" label="N° Nota Fiscal:" placeholder="Selecionar"
            value="{{ request('id_nf_ordem') }}" asyncSearch="false" :options="$nfs" />
        ...
        <x-forms.smart-select name="id_fornecedor" label="Fornecedor:" placeholder="Selecionar"
            value="{{ request('id_fornecedor') }}" asyncSearch="false" :options="$fornecedor" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            {{-- <button type="button" x-on:click="$store.relatorionfsmanutencaorealizadas.gerarPdf()"
                :disabled="$store.relatorionfsmanutencaorealizadas.loading"
                ...
                <span x-show="$store.relatorionfsmanutencaorealizadas.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorionfsmanutencaorealizadas.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorionfsmanutencaorealizadas.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button> --}}

            <button type="button" x-on:click="$store.relatorionfsmanutencaorealizadas.gerarExcel()"
                :disabled="$store.relatorionfsmanutencaorealizadas.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatorionfsmanutencaorealizadas.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorionfsmanutencaorealizadas.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
