    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editar Tipo Fornecedor') }}: {{ $tipofornecedor->descricao_tipo }}
            </h2>
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                ...
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode editar as informações de um tipo de fornecedor existente.
    ...
    <div>
        <div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.tipofornecedores.update', $tipofornecedor) }}" method="POST"
                        x-data="{ isSubmitting: false }"
                        x-on:submit="if (!isSubmitting) { isSubmitting = true; $el.submit(); }">
                        @csrf
                        @method('PUT')
