<div class="space-y-6">
    <div class="bg-gray-50 p-4 rounded-lg">
        <div class="flex gap-2 mb-5">
            <div class="w-64 mt-5">
                <x-forms.input label="Número do Pedido" name="id_pedido_compras" id="id_pedido_compra_b"
                    value="{{ old('id_pedido_compras', $nfEntrada->id_pedido_compras ?? '') }}" />
            </div>
            <div class="mt-12">
                <x-forms.button onclick="buscaPedido()">
                    Buscar Pedido
                </x-forms.button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <x-forms.input label="Código Entrada" name="id_nota_fiscal_entrada" :disabled="true"
                value="{{ old('id_nota_fiscal_entrada', $nfEntrada->id_nota_fiscal_entrada ?? 0) }}" />

            <x-forms.input label="CNPJ" id="cnpj" name="cnpj"
                value="{{ old('cnpj', $nfEntrada->cnpj ?? '') }}" />

            <x-forms.input label="Código Fornecedor" name="id_fornecedor" id="id_fornecedor"
                value="{{ old('id_fornecedor', $nfEntrada->id_fornecedor ?? '') }}" />

            <x-forms.input label="Nome Empresa" name="nome_empresa" id="nome_empresa"
                value="{{ old('nome_empresa', $nfEntrada->nome_empresa ?? '') }}" />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 mt-2">
            <x-forms.input label="Endereço" name="endereco" id="endereco"
                value="{{ old('endereco', $nfEntrada->endereco ?? '') }}" />

            <x-forms.input label="Bairro" name="bairro" id="bairro"
                value="{{ old('bairro', $nfEntrada->bairro ?? '') }}" />

            <x-forms.input label="Número" name="numero" id="numero"
                value="{{ old('numero', $nfEntrada->numero ?? '') }}" />

            <x-forms.smart-select label="Municipio" name="nome_municipio" :options="$municipios" :selected="old('nome_municipio', $nfEntrada->nome_municipio ?? '')" />

            <x-forms.smart-select label="Estado" name="uf" :options="$estados" :selected="old('uf', $nfEntrada->uf ?? '')" />

            <x-forms.input label="CEP" name="cep" value="{{ old('cep', $nfEntrada->cep ?? '') }}" />
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-7 gap-4 mt-2">
            <div class="col-span-2">
                <x-forms.input label="Chave Nota Fiscal" name="chave_nf_entrada" id="chave_nf_entrada"
            ...
            <x-forms.input label="Código Nota Fiscal" name="cod_nota_fiscal"
                value="{{ old('cod_nota_fiscal', $nfEntrada->cod_nota_fiscal ?? '') }}" />

            <x-forms.input label="Número Nota Fiscal" name="numero_nota_fiscal" id="numero_nota_fiscal"
                value="{{ old('numero_nota_fiscal', $nfEntrada->numero_nota_fiscal ?? '') }}" />

            <x-forms.input label="Natureza Operação" name="natureza_operacao"
                value="{{ old('natureza_operacao', $nfEntrada->natureza_operacao ?? '') }}" />


            <x-forms.input type="datetime-local" label="Data de Emissão" name="data_emissao"
                value="{{ old('data_emissao', $nfEntrada->data_emissao ?? '') }}" />

            <x-forms.input type="datetime-local" label="Data de Saída" name="data_saida"
                value="{{ old('data_saida', $nfEntrada->data_saida ?? '') }}" />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mt-2">
            <x-forms.input label="Valor Nota Fiscal" name="valor_nota_fiscal"
                value="{{ old('valor_nota_fiscal', $nfEntrada->valor_nota_fiscal ?? '') }}" />

            <x-forms.input label="Valor Desconto" name="valor_desconto_nfe"
                value="{{ old('valor_desconto', $nfEntrada->valor_desconto ?? '') }}" />

            <x-forms.input label="Valor Frete" name="valor_frete"
                value="{{ old('valor_frete', $nfEntrada->valor_frete ?? '') }}" />

            <x-forms.smart-select label="Filial" name="id_filial" :options="$filial" :selected="old('id_filial', $nfEntrada->id_filial ?? '')" />
            ...
            <div class="relative inline-flex flex-col mt-1">
                ...
                <div class="flex">
                    <div class="relative">
                        <input type="radio" name="aplica_rateio" value="1" id="aplica_rateio_sim"
                    ...
                    <div class="relative">
                        <input type="radio" name="aplica_rateio" value="0" id="aplica_rateio_nao"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
            <x-forms.input label="Código de Produto" name="cod_produto" value="{{ old('cod_produto') }}" />

            <x-forms.input label="Nome Produto" name="nome_produto" value="{{ old('nome_produto') }}" />

            <x-forms.input label="NCM" name="ncm" value="{{ old('ncm') }}" />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mt-2">
            <x-forms.input label="Unidade" name="unidade" value="{{ old('unidade') }}" />

            <x-forms.input label="Quantidade Produtos" name="quantidade_produtos"
                value="{{ old('quantidade_produtos') }}" />

            <x-forms.input label="Valor Unitário" name="valor_unitario" value="{{ old('valor_unitario') }}" />

            <x-forms.input label="Valor Total" name="valor_total" value="{{ old('valor_total') }}" />

            <x-forms.input label="Valor Total com Desconto" name="valor_desconto_produtos" id="campo_valor_desconto"
                value="{{ old('valor_desconto_produtos') }}" />
        ...
        <div class="p-6 bg-white border-gray-200 mt-5">
            ...
            <!-- Campos hidden opcionais para compatibilidade -->
...
@push('scripts')
    ...
    {{-- <script src="{{ asset('js/notafiscalentrada/teste.js') }}"></script> --}}
