<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriobaixaestoque', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_produto: document.querySelector('select[name="id_produto"], #id_produto')?.value || '',
                    id_estoque: document.querySelector('select[name="id_estoque"], #id_estoque')?.value || '',
                    id_grupo: document.querySelector('select[name="id_grupo"], #id_grupo')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriobaixaestoque/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_baixa${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriobaixaestoque/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_baixa${new Date().toISOString().slice(0,10)}.xls`);
