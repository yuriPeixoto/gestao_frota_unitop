<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatoriocertificadoveiculo', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_tipo_certificado: document.querySelector('select[name="id_tipo_certificado"], #id_tipo_certificado')?.value || '',
                };
            },
            ...
            async gerarPdf() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocertificadoveiculo/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_certificado${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                   ...
                   if (!this.validarDatas(dados)) return;
                    ...
                    const response = await this.fazerRequisicao('/admin/relatoriocertificadoveiculo/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_certificado${new Date().toISOString().slice(0,10)}.xls`);
