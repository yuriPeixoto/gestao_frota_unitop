<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <div class="flex items-center space-x-4">
                ...
                <x-help-icon title="Ajuda - Editar Encerrante"
                    content="Edite os dados do encerrante. Selecione primeiro o tanque para carregar as bombas disponíveis. Todos os campos são obrigatórios." />
            </div>
        </div>
    </x-slot>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <form action="{{ route('admin.encerrantes.update', $encerrante) }}" method="POST" id="encerrante-form">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-12 gap-4 mb-4">
                    <div class="col-span-3">
                        ...
                        <input type="text" id="id_encerrante" name="id_encerrante"
                            ...
                            value="{{ $encerrante->id_encerrante }}" readonly>
                    </div>
                    ...
                    <div class="col-span-5">
                        ...
                        <select name="id_tanque" id="id_tanque" required
                            ...
                            <option value="{{ $tanque->id_tanque }}" {{ (old('id_tanque', $encerrante->id_tanque) ==
                                $tanque->id_tanque) ? 'selected' : '' }}>
                                {{ $tanque->tanque }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-4">
                        ...
                        <select name="id_bomba" id="id_bomba" required
                            ...
                            @if($encerrante->bomba)
                            <option value="{{ $encerrante->bomba->id_bomba }}" selected>
                ...
                <div class="grid grid-cols-12 gap-4 mb-4">
                    <div class="col-span-6">
                        ...
                        <input type="datetime-local" name="data_hora_abertura" id="data_hora_abertura" required
                            value="{{ old('data_hora_abertura', $encerrante->data_hora_abertura ? $encerrante->data_hora_abertura->format('Y-m-d\TH:i') : '') }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="number" name="encerrante_abertura" id="encerrante_abertura" required min="0"
                            value="{{ old('encerrante_abertura', $encerrante->encerrante_abertura) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="grid grid-cols-12 gap-4 mb-4">
                    <div class="col-span-6">
                        ...
                        <input type="datetime-local" name="data_hora_encerramento" id="data_hora_encerramento" required
                            value="{{ old('data_hora_encerramento', $encerrante->data_hora_encerramento ? $encerrante->data_hora_encerramento->format('Y-m-d\TH:i') : '') }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="number" name="encerrante_fechamento" id="encerrante_fechamento" required min="0"
                            value="{{ old('encerrante_fechamento', $encerrante->encerrante_fechamento) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="grid grid-cols-12 gap-4 mb-6">
                    <div class="col-span-6">
                        ...
                        <input type="text" id="id_filial_display"
                            ...
                            value="{{ $encerrante->filial->name ?? 'Filial não definida' }}" readonly>
                        <input type="hidden" name="id_filial" value="{{ $encerrante->id_filial }}">
                    </div>
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="text" id="usuario_display"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100"
                            value="{{ $encerrante->conferente->name ?? 'Usuário não encontrado' }}" readonly>
                        <input type="hidden" name="usuario" value="{{ $encerrante->usuario }}">
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Armazenar bomba selecionada inicialmente
            const bombaInicialId = '{{ $encerrante->id_bomba }}';
                    ...
                    fetch(`/admin/encerrantes/bombas-por-tanque/${tanqueId}`)
                        ...
                        .then(data => {
                            ...
                            data.forEach(bomba => {
                                ...
                                // Manter bomba selecionada se for a mesma do registro
                                if (bomba.id_bomba == bombaInicialId) {
            ...
            // Carregar bombas iniciais se tanque já estiver selecionado
            if (tanqueSelect.value) {
                tanqueSelect.dispatchEvent(new Event('change'));
            }
