<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_base_veiculo" label="Base do Veiculo:" placeholder="Selecionar" :options="$base"
            value="{{ request('id_base_veiculo') }}" :searchUrl="route('admin.api.baseveiculo.search')"
            asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriopneusaplicado.gerarPdf()"
                :disabled="$store.relatoriopneusaplicado.loading"
                ...
                <span x-show="$store.relatoriopneusaplicado.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopneusaplicado.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopneusaplicado.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriopneusaplicado.gerarExcel()"
                :disabled="$store.relatoriopneusaplicado.loading"
                ...
                <span x-show="$store.relatoriopneusaplicado.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriopneusaplicado.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriopneusaplicado.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
