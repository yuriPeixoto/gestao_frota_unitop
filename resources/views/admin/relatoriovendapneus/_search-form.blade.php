<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id_requisicao_pneu" label="Cód. Requisição:" placeholder="Selecionar"
            ...
            :searchUrl="route('admin.api.requisicao.search')" asyncSearch="false" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatoriovendapneus.gerarPdf()"
                :disabled="$store.relatoriovendapneus.loading"
                ...
                <span x-show="$store.relatoriovendapneus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriovendapneus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriovendapneus.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatoriovendapneus.gerarExcel()"
                :disabled="$store.relatoriovendapneus.loading"
                ...
                <span x-show="$store.relatoriovendapneus.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatoriovendapneus.loading" class="h-4 w-4 mr-2" />
                <span x-text="$store.relatoriovendapneus.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
