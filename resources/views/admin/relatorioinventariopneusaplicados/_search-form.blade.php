<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <x-forms.smart-select name="id" label="Cód. Inventário:" placeholder="Selecionar" :options="$invetario"
            value="{{ request('id') }}" />
    ...
    <div class="flex justify-between mt-4">
        ...
        <div class="flex space-x-2">
            ...
            <button type="button" x-on:click="$store.relatorioinventariopneusaplicados.gerarPdf()"
                :disabled="$store.relatorioinventariopneusaplicados.loading"
                ...
                <span x-show="$store.relatorioinventariopneusaplicados.loading" class="loading-spinner mr-2"></span>
                <x-icons.magnifying-glass x-show="!$store.relatorioinventariopneusaplicados.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioinventariopneusaplicados.loading ? 'Gerando...' : 'Buscar PDF'"></span>
            </button>

            <button type="button" x-on:click="$store.relatorioinventariopneusaplicados.gerarExcel()"
                :disabled="$store.relatorioinventariopneusaplicados.loading"
                ...
                <x-icons.magnifying-glass x-show="!$store.relatorioinventariopneusaplicados.loading"
                    class="h-4 w-4 mr-2" />
                <span x-text="$store.relatorioinventariopneusaplicados.loading ? 'Gerando...' : 'Buscar Excel'"></span>
            </button>
