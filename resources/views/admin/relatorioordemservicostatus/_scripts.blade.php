<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('relatorioOrdemServico', {
            loading: false,
            ...
            coletarDadosFormulario() {
                return {
                    ...
                    id_status_ordem_servico: document.querySelector('select[name="id_status_ordem_servico"], #id_status_ordem_servico')?.value || '',
                    id: document.querySelector('select[name="id"], #id')?.value || '',
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioordemservicostatus/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `ordem_servico_por_status_v1${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/relatorioordemservicostatus/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `ordem_servico_por_status_v1${new Date().toISOString().slice(0,10)}.xls`);
