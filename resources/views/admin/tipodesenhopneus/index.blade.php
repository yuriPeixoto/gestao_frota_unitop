<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Tipo de Desenho Pneu') }}
            </h2>
            <div class="flex items-center space-x-4">
                <x-button-link href="{{ route('admin.tipodesenhopneus.create') }}"
                ...
                <div class="relative inline-block text-left" x-data="{ helpOpen: false }">
                    ...
                    <div x-show="helpOpen" @click.away="helpOpen = false"
                        ...
                        <div class="py-1">
                            <div class="px-4 py-2">
                                <p class="text-sm leading-5 font-medium text-gray-900 truncate">Ajuda - Tipo de Desenho
                                </p>
                                <p class="mt-1 text-xs leading-5 text-gray-500">
                                    Nesta tela você pode visualizar todos os tipos de desenho de pneus cadastrados.
                                    Utilize o botão 'Novo Tipo' para adicionar um novo registro. Você pode editar ou
                                    excluir tipos de desenhos existentes utilizando as ações disponíveis em cada linha
                                    da tabela.
                                </p>
                            </div>
                        </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-4 bg-white border-b border-gray-200">
            ...
            <!-- Search Form -->
            @include('admin.tipodesenhopneus._search-form')
                ...
                <div id="results-table">
                    <x-tables.table>
                        <x-tables.header>
                            ...
                            <x-tables.head-cell>Número de Sulcos</x-tables.head-cell>
                            <x-tables.head-cell>Quantidade de Lona</x-tables.head-cell>
                            <x-tables.head-cell>Dias Calibragem</x-tables.head-cell>
                        ...
                        <x-tables.body>
                            @forelse ($tipodesenhopneus as $index => $tipo)
                                <x-tables.row :index="$index" data-id="{{ $tipo['id'] }}">
                                    <x-tables.cell>{{ $tipo['id'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['descricao'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['Número de Sulcos'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['Quantidade de Lona'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['Dias Calibragem'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['Data Inclusão'] }}</x-tables.cell>
                                    <x-tables.cell>{{ $tipo['Data Alteração'] }}</x-tables.cell>
                                    <x-tables.cell>
                                        <div class="flex items-center space-x-2">
                                            {{-- <button type="button" onclick="visualizarTipoDesenho({{ $tipo['id'] }})"
                                            ...
                                            <button type="button" onclick="editarTipoDesenho({{ $tipo['id'] }})"
    ...
    @push('scripts')
        <script>
            ...
            // Função para visualizar tipo de desenho
            function visualizarTipoDesenho(id) {
                window.location.href = `{{ route('admin.tipodesenhopneus.show', ':id') }}`.replace(':id', id);
            }

            // Função para editar tipo de desenho
            function editarTipoDesenho(id) {
                window.location.href = `{{ route('admin.tipodesenhopneus.edit', ':id') }}`.replace(':id', id);
            }
            ...
            function executarExclusao() {
                ...
                fetch(`{{ route('admin.tipodesenhopneus.destroy', ':id') }}`.replace(':id', idExclusao), {
                        method: 'DELETE',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        // Se quiser que a pessoa veja a mensagem, remova isso ou aumente o tempo
                        setTimeout(() => {
                    ...
                    .catch(error => {
                        ...
                        showNotification(
                            'Erro',
                            error.message || 'Não foi possível excluir o tipo de desenho',
                            'error'
                        );
                    })
                    .finally(() => {
