<script>
    document.addEventListener('alpine:init', () => {
        Alpine.store('abastecimentoporbomposto', {
            loading: false,
            ...
            async gerarPdf() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/abastecimentoporbomposto/gerarpdf', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_abastecimentoporbomba${new Date().toISOString().slice(0,10)}.pdf`);
            ...
            async gerarExcel() {
                ...
                try {
                    ...
                    const response = await this.fazerRequisicao('/admin/abastecimentoporbomposto/gerarexcel', dados);
                    const blob = await response.blob();

                    this.fazerDownload(blob, `relatorio_abastecimentoporbomba${new Date().toISOString().slice(0,10)}.xls`);
