...
class RelatorioVeiculos extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('is_ativo')) {
            $query->where('is_ativo', $request->input('is_ativo'));
        }
        ...
        return view('admin.relatorioveiculos.index', compact('filial', 'departamento', 'equipamento', 'placa', 'ativo'));
    }
    ...
    public function gerarPdf(Request $request)
    {
        try {
            ...
            // Produtos
            $veiculo       = empty($request['id_veiculo']) ? "0" : $request['id_veiculo'];
            $filial        = empty($request['id_filial']) ? "0" : $request['id_filial'];
            $departamento  = empty($request['id_departamento']) ? "0" : $request['id_departamento'];
            $equipamento   = empty($request['id_tipo_equipamento']) ? "0" : $request['id_tipo_equipamento'];
            //$terceiro      = $request['terceiro'];
            $situacao      = $request['situacao_veiculo'];
            ...
            if (!empty($request['situacao_veiculo'])) {
                $is_situacao  = 'in';
                $id_situacao  = $request['situacao_veiculo'];
            } else {
                $is_situacao  = 'in';
                $id_situacao  = 'true,false';
            }
