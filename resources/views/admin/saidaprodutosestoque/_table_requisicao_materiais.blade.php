        ...
        <table class="w-full min-w-[1200px] divide-y divide-gray-200 text-left text-sm text-gray-700">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-xs font-medium uppercase text-gray-500">Código Solicitante</th>
                    ...
                    <th class="px-3 py-2 text-xs font-medium uppercase text-gray-500">Placa Veículo</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($relacaoMats as $key => $item)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ $item->pessoalEstoque->name ?? ($item->pessoalEstoque->nome ?? $item->id_usuario_estoque) }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ $item->transferenciaEstoqueAux->first()?->filialSolicitante?->name ?? '' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ $item->transferenciaEstoqueAux->first()?->id_relacao_solicitacoes_antigo ?? ' ' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ $item->situacao }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ date('d/m/Y', strtotime($item->data_aprovacao)) }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
